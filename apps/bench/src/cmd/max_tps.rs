...
use magnus_provider::{
    MagnusNetwork, primitives::MagnusTxEnvelope, provider::ext::MagnusProviderBuilderExt,
};
        ...
        join_all(
            signer_providers
                ...
                .map(async |(_, provider)| {
                    IFeeManagerInstance::new(MIP_FEE_MANAGER_ADDRESS, provider.clone())
...
async fn send_transactions<F: TxFiller<MagnusNetwork> + 'static>(
    transactions: Vec<Vec<u8>>,
    signer_provider_manager: SignerProviderManager<F>,
    max_concurrent_requests: usize,
    tps: u64,
    deadline: Sleep,
) -> VecDeque<PendingTransactionBuilder<MagnusNetwork>> {
    info!(
...
async fn generate_transactions<F: TxFiller<MagnusNetwork> + 'static>(
    ...
    let tx_weights: [u64; TX_TYPES] = [mip20_weight, swap_weight, place_order_weight, erc20_weight];
    ...
    let mip20_transfers = Arc::new(AtomicUsize::new(0));
    ...
    let builders = ProgressBar::new(total_txs)
        ...
        .map(async |provider| {
            ...
            let mut tx = match tx_index {
                0 => {
                    mip20_transfers.fetch_add(1, Ordering::Relaxed);
    ...
    info!(
        ...
        mip20_transfers = mip20_transfers.load(Ordering::Relaxed),
    ...
    let transactions = builders
        .into_par_iter()
        .progress()
        .map(|(tx, signer)| -> eyre::Result<MagnusTxEnvelope> {
...
struct GenerateTransactionsInput<F: TxFiller<MagnusNetwork>> {
    total_txs: u64,
    accounts: u64,
    signer_provider_manager: SignerProviderManager<F>,
    max_concurrent_requests: usize,
