...
type SignedProviderFactory = Box<
    dyn Fn(PrivateKeySigner, Url, CachedNonceManager) -> DynProvider<MagnusNetwork> + Send + Sync,
>;
...
struct SignerProviderManagerInner<F: TxFiller<MagnusNetwork>> {
    ...
    signer_providers: Vec<(PrivateKeySigner, DynProvider<MagnusNetwork>)>,
}
...
impl<F: TxFiller<MagnusNetwork> + 'static> SignerProviderManager<F> {
    ...
    pub fn signer_providers(&self) -> &[(PrivateKeySigner, DynProvider<MagnusNetwork>)] {
