//! Main executable for the Magnus node.
//!
...
//! - Magnus consensus engine for block agreement
//!
//! The node operates by:
//! 1. Starting the Reth node infrastructure (database, networking, RPC)
//! 2. Creating the application state that bridges Reth and consensus
//! 3. Launching the Magnus consensus engine via a separate task and a separate tokio runtime.
...
use magnus_consensus_engine::{feed as consensus_feed, run_consensus_stack};
use magnus_consensus::MagnusConsensus;
use magnus_evm::{MagnusEvmConfig, MagnusEvmFactory};
use magnus_faucet::{
    args::FaucetArgs,
...
struct MagnusArgs {
    ...
    #[command(flatten)]
    pub consensus: magnus_consensus_engine::Args,
...
fn main() -> eyre::Result<()> {
    ...
    if let Some(result) = magnus_cmd::try_run_magnus_subcommand() {
        return result;
    }

    let cli = Cli::<MagnusChainSpecParser, MagnusArgs>::parse();
    ...
    let consensus_handle = thread::spawn(move || {
        ...
        let ret = if node.config.dev.dev || args.follow.is_some() {
            ...
            runner.start(async move |ctx| {
                ...
                let mut metrics_server = magnus_consensus_engine::metrics::install(
    ...
    let components = |spec: Arc<MagnusChainSpec>| {
        (
            MagnusEvmConfig::new(spec.clone(), MagnusEvmFactory::default()),
            MagnusConsensus::new(spec),
        )
    };
