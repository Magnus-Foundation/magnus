...
use magnus_precompile_registry::{
    ...
    mip_fee_manager::IMIPFeeAMM::{self, IMIPFeeAMMInstance, Mint, Pool},
    mip20::IMIP20,
};
...
pub struct Monitor {
    rpc_url: Url,
    poll_interval: u64,
    tokens: HashMap<Address, MIP20Token>,
...
impl MonitorConfig {
    ...
    async fn fetch_token_metadata<P: Provider + Clone>(
        &self,
        provider: &Arc<P>,
    ) -> Result<HashMap<Address, MIP20Token>> {
    ...
    async fn discover_pools<P: Provider + Clone>(
        ...
        let check_pool_futures: Vec<_> = self
            ...
            .map(|pair| {
                ...
                let fee_amm = IMIPFeeAMM::new(MIP_FEE_MANAGER_ADDRESS, provider.clone());
                async move {
...
impl Monitor {
    ...
    async fn check_for_new_pools(&mut self) -> Result<()> {
        ...
        let filter = Filter::new()
            .address(MIP_FEE_MANAGER_ADDRESS)
    ...
    #[instrument(name = "monitor::update_mip20_pools", skip(self))]
    async fn update_mip20_pools(&mut self) -> Result<()> {
        let provider = ProviderBuilder::new()
            .connect(self.rpc_url.as_str())
            .await?;

        let fee_amm: IMIPFeeAMMInstance<_, _> = IMIPFeeAMM::new(MIP_FEE_MANAGER_ADDRESS, provider);
    ...
    pub async fn worker(&mut self) {
        loop {
            ...
            if let Err(e) = self.update_mip20_pools().await {
