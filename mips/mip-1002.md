...
# MIP-1002: Prevent crossed orders and allow same-tick flip orders

## Abstract

This MIP makes two related changes to the Stablecoin DEX:
...
The `place` and `placeFlip` functions (including the `bookKey` overloads from MIP-1001) are modified to check for crossing before accepting an order:
...
## Interaction with MIP-1001

If MIP-1001 is accepted, the crossing check only applies when the pair is **active**—that is, when the pair's quote token equals the base token's current `quoteToken()`.
...
- On inactive pairs (per MIP-1001), no crossing check is enforced
