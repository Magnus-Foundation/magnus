...
# MIP-1007: Fee Token Introspection

## Abstract

MIP-1007 adds a `getFeeToken()` view function to the FeeManager precompile that returns the fee token address being used for the current transaction. This enables smart contracts to introspect which MIP-20 token is paying for gas fees during execution, allowing for dynamic logic based on the fee token choice.

## Motivation

Magnus transactions support paying gas fees in any USD-denominated MIP-20 token via the fee token preference system. However, prior to this MIP, there was no way for a smart contract to determine which fee token is being used for the current transaction during execution.
...
interface IFeeManager {
    ...
    /// @return The address of the MIP-20 token paying for gas fees
...
contract FeeTokenAware {
    ...
    address constant MAGNUS_USD = 0x20C0000000000000000000000000000000000000;

    function doSomething() external {
        address feeToken = FEE_MANAGER.getFeeToken();

        if (feeToken == MAGNUS_USD) {
            // User is paying fees in MagnusUSD
...
6. **Different fee tokens**: Works with various MIP-20 fee tokens (MagnusUSD, USDC, etc.)
