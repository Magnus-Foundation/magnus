...
contract MIP20 is IMIP20, MIP20RolesAuth {
    ...
    IMIP20 public override quoteToken;
    IMIP20 public override nextQuoteToken;
    ...
    function changeTransferPolicyId(uint64 newPolicyId) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (!MIP403_REGISTRY.policyExists(newPolicyId)) {
    ...
    function burnBlocked(address from, uint256 amount) external onlyRole(BURN_BLOCKED_ROLE) {
        ...
        // MIP-1015: Only allow burning from addresses that are blocked from sending
        if (MIP403_REGISTRY.isAuthorizedSender(transferPolicyId, from)) {
    ...
    function _mint(ResolvedRecipient memory recipient, uint256 amount) internal {
        // MIP-1015: Use mint recipient authorization
        if (!MIP403_REGISTRY.isAuthorizedMintRecipient(transferPolicyId, recipient.target)) {
            revert PolicyForbids();
        }
    ...
    /// @dev In the Magnus node implementation, this function is not exposed via the MIP20 interface
    ...
    function _validateTransfer(address from, ResolvedRecipient memory recipient) internal view {
        ...
        if (
            !MIP403_REGISTRY.isAuthorizedSender(transferPolicyId, from)
                || !MIP403_REGISTRY.isAuthorizedRecipient(transferPolicyId, recipient.target)
        ) {
            revert PolicyForbids();
        }
    }
    ...
    function distributeReward(uint256 amount) external virtual notPaused {
        ...
        // MIP-1015: Use directional authorization for sender -> contract transfer
        if (
            !MIP403_REGISTRY.isAuthorizedSender(transferPolicyId, msg.sender)
                || !MIP403_REGISTRY.isAuthorizedRecipient(transferPolicyId, address(this))
        ) {
            revert PolicyForbids();
        }
    ...
    function setRewardRecipient(address newRewardRecipient) external virtual notPaused {
        ...
        if (newRewardRecipient != address(0)) {
            ...
            if (
                !MIP403_REGISTRY.isAuthorizedSender(transferPolicyId, msg.sender)
                    || !MIP403_REGISTRY.isAuthorizedRecipient(transferPolicyId, newRewardRecipient)
            ) revert PolicyForbids();
        }
    ...
    function claimRewards() external virtual notPaused returns (uint256 maxAmount) {
        // MIP-1015: Use directional authorization for contract -> recipient transfer
        if (
            !MIP403_REGISTRY.isAuthorizedSender(transferPolicyId, address(this))
                || !MIP403_REGISTRY.isAuthorizedRecipient(transferPolicyId, msg.sender)
        ) {
            revert PolicyForbids();
        }
