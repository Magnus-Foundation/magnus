...
contract MagnusStreamChannel is IMagnusStreamChannel, EIP712 {
     ...
     * @param token MIP-20 token address
        ...
        returns (bytes32 channelId)
    {
        ...
        bool success = IMIP20(token).transferFrom(msg.sender, address(this), deposit);
        if (!success) {
        ...
        override
    {
        ...
        bool success = IMIP20(channel.token).transfer(channel.payee, delta);
        if (!success) {
            revert TransferFailed();
        }
    ...
    function topUp(bytes32 channelId, uint256 additionalDeposit) external override {
        ...
        bool success =
            IMIP20(channel.token).transferFrom(msg.sender, address(this), additionalDeposit);
        if (!success) {
            revert TransferFailed();
        }
        ...
        override
    {
        ...
        // Interactions
        if (delta > 0) {
            bool success = IMIP20(token).transfer(payee, delta);
            if (!success) {
                revert TransferFailed();
            }
        }
        ...
        if (refund > 0) {
            bool success = IMIP20(token).transfer(payer, refund);
            if (!success) {
                revert TransferFailed();
            }
        }
