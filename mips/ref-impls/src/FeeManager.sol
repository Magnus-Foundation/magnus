...
contract FeeManager is IFeeManager, FeeAMM {
    ...
    function collectFeePreTx(address user, address userToken, uint256 maxAmount) external {
        ...
        uint64 policyId = IMIP20(userToken).transferPolicyId();
        if (
        ...
        IMIP20(userToken).transferFeePreTx(user, maxAmount);
        ...
        external
    {
        ...
        IMIP20(userToken).transferFeePostTx(user, refundAmount, actualUsed);
