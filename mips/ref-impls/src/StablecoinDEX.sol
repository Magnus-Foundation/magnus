...
contract StablecoinDEX is IStablecoinDEX {
    ...
    function _checkTransferPolicy(address token, address account) internal view returns (bool) {
        // MIP-1015: Check sender authorization for account, recipient for DEX
        uint64 policyId = IMIP20(token).transferPolicyId();
        return MIP403_REGISTRY.isAuthorizedSender(policyId, account)
            && MIP403_REGISTRY.isAuthorizedRecipient(policyId, address(this));
    }
    ...
    function createPair(address base) external returns (bytes32 key) {
        ...
        address quote = address(IMIP20(base).quoteToken());
        // Only USD-denominated tokens are supported, and their quotes must also be USD
        if (keccak256(bytes(IMIP20(base).currency())) != keccak256(bytes("USD"))) {
            revert IMIP20.InvalidCurrency();
        }
        if (keccak256(bytes(IMIP20(quote).currency())) != keccak256(bytes("USD"))) {
            revert IMIP20.InvalidCurrency();
        }
        ...
        returns (uint128 orderId)
    {
        ...
        }
        {
            ...
            if (userBalance >= escrowAmount) {
                ...
                IMIP20(escrowToken).transferFrom(maker, address(this), escrowAmount - userBalance);
            } else {
    ...
    function place(
        ...
        returns (uint128 orderId)
    {
        address quote = address(IMIP20(token).quoteToken());
    ...
    /// @notice Cancel an order where the maker is forbidden by MIP-403 policy
    ...
    function cancelStaleOrder(uint128 orderId) external {
        ...
        // MIP-1015: Check if maker is forbidden from sending by the token's transfer policy
        uint64 policyId = IMIP20(token).transferPolicyId();
        if (MIP403_REGISTRY.isAuthorizedSender(policyId, order.maker)) {
    ...
    function withdraw(address token, uint128 amount) external {
        ...
        IMIP20(token).transfer(msg.sender, amount);
    }
    ...
    function _decrementBalanceOrTransferFrom(address user, address token, uint128 amount) internal {
        // MIP-1015: Check sender authorization for user, recipient for DEX
        uint64 policyId = IMIP20(token).transferPolicyId();
        if (
            !MIP403_REGISTRY.isAuthorizedSender(policyId, user)
                || !MIP403_REGISTRY.isAuthorizedRecipient(policyId, address(this))
        ) {
            revert IMIP20.PolicyForbids();
        }
        ...
        if (userBalance >= amount) {
            ...
            IMIP20(token).transferFrom(user, address(this), remaining);
        }
    }
        ...
        returns (uint128 amountIn)
    {
        ...
        IMIP20(tokenOut).transfer(msg.sender, amountOut);
    }
        ...
        returns (bytes32[] memory, bool[] memory)
    {
        ...
        address inQuote = address(IMIP20(tokenIn).quoteToken());
        address outQuote = address(IMIP20(tokenOut).quoteToken());
        ...
        returns (bytes32[] memory bookKeys, bool[] memory baseForQuote)
    {
        ...
        for (uint256 i = 0; i < path.length - 1; i++) {
            ...
            // MIP-20 quoteToken relationships. While we are "going up" the tree,
            ...
            if (isBaseForQuote && address(IMIP20(hopTokenIn).quoteToken()) == hopTokenOut) {
                ...
                if (address(IMIP20(hopTokenOut).quoteToken()) == hopTokenIn) {
