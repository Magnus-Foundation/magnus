...
contract FeeAMMInvariantTest is InvariantBaseTest {
        ...
        returns (address actor, uint256 balance)
    {
        ...
        for (uint256 i = 0; i < BLACKLISTABLE_ACTOR_COUNT; i++) {
            ...
            if (!registry.isAuthorized(policyId, a)) {
                uint256 bal = MIP20(token).balanceOf(a);
    ...
    function mint(uint256 actorSeed, uint256 pairSeed, uint256 amount) external {
        ...
        _ensureFunds(actor, MIP20(validatorToken), amount);
    ...
    function burn(uint256 actorSeed, uint256 pairSeed, uint256 liquidityPct) external {
        ...
        ctx.actorUserBalanceBefore = MIP20(ctx.userToken).balanceOf(ctx.actor);
        ctx.actorValidatorBalanceBefore = MIP20(ctx.validatorToken).balanceOf(ctx.actor);
    ...
    function rebalanceSwap(uint256 actorSeed, uint256 pairSeed, uint256 amountOutRaw) external {
        ...
        _ensureFunds(ctx.actor, MIP20(ctx.validatorToken), ctx.expectedAmountIn * 2);

        ctx.actorValidatorBefore = MIP20(ctx.validatorToken).balanceOf(ctx.actor);
        ctx.actorUserBefore = MIP20(ctx.userToken).balanceOf(ctx.actor);
    ...
    function mintBurnCycle(uint256 actorSeed, uint256 pairSeed, uint256 amount) external {
        ...
        uint256 actorBalBefore = MIP20(validatorToken).balanceOf(actor);
    ...
    function smallRebalanceSwap(uint256 actorSeed, uint256 pairSeed) external {
        ...
        _ensureFunds(actor, MIP20(validatorToken), expectedIn * 2);
    ...
    function tryFirstMintBoundary(uint256 actorSeed, uint256 pairSeed) external {
        ...
        _ensureFunds(actor, MIP20(validatorToken), boundaryAmount);
        ...
        _ensureFunds(actor, MIP20(validatorToken), aboveBoundary);
    ...
    /// @dev On magnus-foundry, fees are only collected via protocol tx execution
    ...
    function distributeFees(uint256 seed) external {
        ...
        uint256 validatorBalanceBefore = MIP20(token).balanceOf(validator);
    ...
    ///      (Solidity reference impl) and magnus-foundry (Rust precompile).
        ...
        external
    {
        ...
        if (userToken != validatorToken) {
            ...
            _ensureFunds(user, MIP20(userToken), feeAmount);
            vm.prank(user);
            try MIP20(userToken).transfer(address(amm), feeAmount) returns (bool success) {
                assertTrue(success);
