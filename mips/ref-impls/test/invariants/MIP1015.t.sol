...
contract MIP1015InvariantTest is InvariantBaseTest {
    ...
    mapping(uint64 => IMIP403Registry.PolicyType) private _policyTypes;
    ...
    function setUp() public override {
        ...
        for (uint256 i = 0; i < 4; i++) {
            IMIP403Registry.PolicyType ptype = i % 2 == 0
        ...
        _policyTypes[compoundPid] = IMIP403Registry.PolicyType.COMPOUND;
    ...
    function createSimplePolicy(uint256 actorSeed, bool isWhitelist) external {
        ...
        IMIP403Registry.PolicyType ptype = isWhitelist
            ? IMIP403Registry.PolicyType.WHITELIST
            : IMIP403Registry.PolicyType.BLACKLIST;
        ...
        external
    {
        ...
        try registry.createCompoundPolicy(sPid, rPid, mPid) returns (uint64 compoundPid) {
            ...
            (IMIP403Registry.PolicyType ptype, address policyAdmin) =
    ...
    function modifySimplePolicy(uint256 policySeed, uint256 accountSeed, bool add) external {
        ...
        if (ptype == IMIP403Registry.PolicyType.WHITELIST) {
    ...
    function optIntoRewards(uint256 tokenSeed, uint256 actorSeed) external {
        ...
        MIP20 token = _compoundTokens[tokenSeed % _compoundTokens.length];
        ...
        public
    {
        ...
        if (authorized) {
            ...
            assertEq(
                errorSelector, IMIP20.PolicyForbids.selector, "Wrong error for unauthorized mint"
            );
        }
        ...
        external
    {
        ...
        if (senderAuth && recipientAuth) {
            ...
            try token.transfer(recipient, amount) {
                ...
                assertEq(bytes4(reason), IMIP20.PolicyForbids.selector, "Wrong error for transfer");
            }
        }
    }
        ...
        external
    {
        ...
        if (!senderAuth) {
            ...
            try token.burnBlocked(target, amount) {
                ...
                assertEq(
                    bytes4(reason), IMIP20.PolicyForbids.selector, "Wrong error for burnBlocked"
                );
            }
        }
        vm.stopPrank();
    ...
    function _authorize(uint64 policyId, address account, bool authorize) internal {
        ...
        (IMIP403Registry.PolicyType ptype, address policyAdmin) = registry.policyData(policyId);

        vm.startPrank(policyAdmin);
        if (ptype == IMIP403Registry.PolicyType.WHITELIST) {
            registry.modifyPolicyWhitelist(policyId, account, authorize);
        } else if (ptype == IMIP403Registry.PolicyType.BLACKLIST) {
    ...
    function _assertKnownRegistryRevert(bytes memory reason) internal pure {
        ...
        bool isKnown = selector == IMIP403Registry.Unauthorized.selector
            || selector == IMIP403Registry.IncompatiblePolicyType.selector
            || selector == IMIP403Registry.PolicyNotFound.selector;
