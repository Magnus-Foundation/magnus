...
contract VirtualAddressesInvariantTest is InvariantBaseTest {
    ...
    function _configureVirtualPolicies() internal {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            uint64 mintPolicyId = registry.createPolicy(admin, IMIP403Registry.PolicyType.BLACKLIST);
    ...
    function _setTransferAllowed(MIP20 token, address master, bool allowed) internal {
    ...
    function _setMintAllowed(MIP20 token, address master, bool allowed) internal {
    ...
    function _mintableAmount(MIP20 token) internal view returns (uint256) {
    ...
    function _assertNoRelevantTokenLogs(MIP20 token, string memory message) internal {
        assertEq(_relevantTokenLogs(token).length, 0, message);
    }

    function _relevantTokenLogs(MIP20 token) internal returns (Vm.Log[] memory relevant) {
    ...
    function _isRelevantTokenEvent(bytes32 topic0) internal pure returns (bool) {
        return topic0 == Magnus.TRANSFER_EVENT || topic0 == Magnus.TRANSFER_WITH_MEMO_EVENT
            || topic0 == Magnus.MINT_EVENT;
    }
