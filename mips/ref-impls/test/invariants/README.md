...
- **TEMPO-FAC2**: MIP20 recognition - all tokens created by the factory are recognized as MIP-20 by `isTIP20()`.
- **TEMPO-FAC3**: Address uniqueness - attempting to create a token at an existing address reverts with `TokenAlreadyExists`.
- **TEMPO-FAC4**: Quote token validation - `createToken` reverts with `InvalidQuoteToken` if the quote token is not a valid MIP-20.
...
- **TEMPO-FAC8**: isTIP20 consistency - created tokens return true, non-MIP20 addresses return false.
...
- **TEMPO-FAC11**: Address format - all created tokens have addresses with the correct MIP-20 prefix (`0x20C0...`).
...
- **TEMPO-MIP1**: Balance conservation - sender balance decreases by exactly `amount`, recipient balance increases by exactly `amount`. Transfer returns `true` on success.
- **TEMPO-MIP2**: Total supply unchanged after transfer - transfers only move tokens between accounts.
- **TEMPO-MIP3**: Allowance consumption - `transferFrom` decreases allowance by exactly `amount` transferred.
- **TEMPO-MIP4**: Infinite allowance preserved - `type(uint256).max` allowance remains infinite after `transferFrom`.
- **TEMPO-MIP9**: Memo transfers behave identically to regular transfers for balance accounting.

### Approval Invariants

- **TEMPO-MIP5**: Allowance setting - `approve` sets exact allowance amount, returns `true`.
- **TEMPO-MIP36**: A valid permit sets allowance to the `value` in the permit struct.

### Mint/Burn Invariants

- **TEMPO-MIP6**: Minting increases total supply and recipient balance by exactly `amount`.
- **TEMPO-MIP7**: Supply cap enforcement - minting reverts if `totalSupply + amount > supplyCap`.
- **TEMPO-MIP8**: Burning decreases total supply and burner balance by exactly `amount`.
- **TEMPO-MIP23**: Burn blocked - `burnBlocked` decreases target balance and total supply by exactly `amount` when target is blacklisted.

### Reward Distribution Invariants

- **TEMPO-MIP10**: Reward recipient setting - `setRewardRecipient` updates the stored recipient correctly.
- **TEMPO-MIP11**: Opted-in supply tracking - `optedInSupply` increases when opting in (by holder's balance) and decreases when opting out.
- **TEMPO-MIP25**: Reward delegation - users can delegate their rewards to another address via `setRewardRecipient`.
- **TEMPO-MIP12**: Global reward per token updates - `distributeReward` increases `globalRewardPerToken` by `(amount * ACC_PRECISION) / optedInSupply`.
- **TEMPO-MIP13**: Reward token custody - distributed rewards are transferred to the token contract.
- **TEMPO-MIP14**: Reward claiming - `claimRewards` transfers owed amount from contract to caller, updates balances correctly.
- **TEMPO-MIP15**: Claim bounded by available - claimed amount cannot exceed contract's token balance.

### Policy Invariants

- **TEMPO-MIP16**: Blacklist enforcement - transfers to/from blacklisted addresses revert with `PolicyForbids`.
- **TEMPO-MIP17**: Pause enforcement - transfers revert with `ContractPaused` when paused.

### Global Invariants

- **TEMPO-MIP18**: Supply conservation - `totalSupply = initialSupply + totalMints - totalBurns`.
- **TEMPO-MIP19**: Opted-in supply bounded - `optedInSupply <= totalSupply`.
- **TEMPO-MIP20**: Balance sum equals supply - sum of all holder balances equals `totalSupply`.
- **TEMPO-MIP21**: Decimals constant - `decimals()` always returns 6.
- **TEMPO-MIP22**: Supply cap enforced - `totalSupply <= supplyCap` always holds.

### Protected Address Invariants

- **TEMPO-MIP24**: Protected address enforcement - `burnBlocked` cannot be called on FeeManager or DEX addresses (reverts with `ProtectedAddress`).

### Access Control Invariants

- **TEMPO-MIP26**: Issuer-only minting - only accounts with `ISSUER_ROLE` can call `mint` (non-issuers revert with `Unauthorized`).
- **TEMPO-MIP27**: Pause-role enforcement - only accounts with `PAUSE_ROLE` can call `pause` (non-role holders revert with `Unauthorized`).
- **TEMPO-MIP28**: Unpause-role enforcement - only accounts with `UNPAUSE_ROLE` can call `unpause` (non-role holders revert with `Unauthorized`).
- **TEMPO-MIP29**: Burn-blocked-role enforcement - only accounts with `BURN_BLOCKED_ROLE` can call `burnBlocked` (non-role holders revert with `Unauthorized`).

### Permit Invariants

- **TEMPO-MIP31**: `nonces(owner)` must only ever increase, never decrease.
- **TEMPO-MIP32**: `nonces(owner)` must increment by exactly 1 on each successful `permit()` call for that owner.
- **TEMPO-MIP33**: A permit signature can only be used once (enforced by nonce increment).
...
- **TEMPO-VA15**: Policy-operation rejection - MIP-403 configuration APIs reject virtual aliases as literal policy members.
