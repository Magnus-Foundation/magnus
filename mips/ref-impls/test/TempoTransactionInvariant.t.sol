...
import { VmExecuteTransaction, VmRlp } from "magnus-std/StdVm.sol";
import { Eip1559Transaction, Eip1559TransactionLib } from "magnus-std/tx/Eip1559TransactionLib.sol";
import {
...
contract TempoTransactionInvariantTest is InvariantChecker {
    ...
    function setUp() public override {
        ...
        // NOTE: handler_invalidFeeToken disabled due to BUG-002 (causes magnus-foundry panic)
        ...
        returns (bytes memory signedTx, address sender)
    {
        ...
        if (sigType == SignatureType.Secp256k1 && nonceKey == 0) {
            signedTx = TxBuilder.buildLegacyCall(
                ...
                vm,
                address(feeToken),
                abi.encodeCall(IMIP20.transfer, (to, amount)),
                nonceValue,
    ...
    /// @notice Handler: Execute a real Magnus transaction to increment a 2D nonce key
        ...
        external
    {
        ...
        bytes memory signedTx = TxBuilder.buildTempoCallKeychain(
            ...
            vm,
            address(feeToken),
            abi.encodeCall(IMIP20.transfer, (recipient, amount)),
            nonceKey,
            currentNonce,
            ctx.keyPk,
            ctx.owner
        ...
        external
    {
        ...
        bytes memory signedTx = TxBuilder.buildTempoCreateNotFirst(
            ...
            vm,
            address(feeToken),
            abi.encodeCall(IMIP20.transfer, (ctx.sender, 1e6)),
            initcode,
            ctx.nonceKey,
            ctx.current2dNonce,
    ...
    /// @notice Handler: Attempt CREATE with value > 0 (invalid for Magnus - C4)
    ...
    /// @notice Handler: Attempt to replay a Magnus transaction with same 2D nonce
        ...
        external
    {
        ...
        // Magnus txs with nonceKey > 0 only increment 2D nonce, not protocol nonce
        try vmExec.executeTransaction(signedTx) {
    ...
    /// @notice Handler F6: Verify non-MIP20 fee token is rejected
    /// @dev F6: Non-zero spending requires MIP20 prefix (0x20C0...)
    ...
    /// @dev Creates a Magnus tx with validAfter in the future, expects rejection
    ...
    /// @dev Creates a Magnus tx with validBefore in the past, expects rejection
    ...
    /// @dev Creates a Magnus tx with both bounds set, tests edge cases
    ...
    /// @dev Creates a Magnus tx without time bounds, should always succeed (if other conditions met)
    ...
    /// @dev Creates a Magnus tx with validAfter == validBefore, expects rejection
        ...
        external
    {
        ...
        bytes memory data = abi.encodeCall(IMIP20.transfer, (recipient, amount));
        ...
        external
    {
        ...
        uint64 currentNonce = uint64(ghost_protocolNonce[sender]);
        bytes memory callData = abi.encodeCall(IMIP20.transfer, (recipient, amount));
    ...
    /// @dev Expiring nonce key constant (MIP-1009)
    uint256 private constant EXPIRING_NONCE_KEY = type(uint256).max;
    /// @dev Maximum expiry window in seconds (MIP-1009)
