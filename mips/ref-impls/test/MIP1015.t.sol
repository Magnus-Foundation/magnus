...
/// @title MIP-1015 Compound Policy Tests
/// @notice Unit tests and stateless fuzz tests for compound transfer policies as specified in MIP-1015
/// @dev Tests both MIP403Registry compound policy functions and MIP-20 integration
/// forge-config: default.hardfork = "magnus:T2"
contract MIP1015Test is BaseTest {
    ...
    function setUp() public override {
        ...
        whitelistPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        blacklistPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.BLACKLIST);
        senderOnlyPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        recipientOnlyPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        mintRecipientWhitelist = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        senderBlacklist = registry.createPolicy(admin, IMIP403Registry.PolicyType.BLACKLIST);
    ...
    function test_invariant2_compoundPolicyHasNoAdmin() public {
        ...
        (IMIP403Registry.PolicyType policyType, address policyAdmin) = registry.policyData(cp);
    ...
    function test_invariant3_revertsOnNonExistentPolicy() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IMIP403Registry.PolicyNotFound.selector, nonExistentPolicy)
        );
    ...
    function test_vendorCreditsUseCase() public {
        ...
        uint64 vendorWhitelist = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
    ...
    function test_asymmetricSenderRestriction() public {
        ...
        uint64 senderBL = registry.createPolicy(admin, IMIP403Registry.PolicyType.BLACKLIST);
    ...
    function test_mint_fails_unauthorizedMintRecipient_simplePolicy() public {
        ...
        vm.expectRevert(IMIP20.PolicyForbids.selector);
    ...
    /*//////////////////////////////////////////////////////////////
                    MIP-20 TRANSFER INTEGRATION TESTS
    //////////////////////////////////////////////////////////////*/
    ...
    /*//////////////////////////////////////////////////////////////
                    MIP-20 BURN_BLOCKED INTEGRATION TESTS
    //////////////////////////////////////////////////////////////*/
        ...
        public
    {
        ...
        uint64 sPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        uint64 rPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        ...
        public
    {
        ...
        uint64 sPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        uint64 rPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        uint64 mPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
    ...
    function test_invariant7_distributeRewardRequiresBothAuth() public {
        ...
        uint64 senderWL = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        uint64 recipientWL = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        ...
        public
    {
        ...
        uint64 senderPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
        uint64 recipientPolicy = registry.createPolicy(admin, IMIP403Registry.PolicyType.WHITELIST);
    ...
    function mintExternal(MIP20 token, address to, uint256 amount) external {
        token.mint(to, amount);
    }

    function transferExternal(MIP20 token, address to, uint256 amount) external {
        token.transfer(to, amount);
    }

    function transferFromExternal(MIP20 token, address from, address to, uint256 amount) external {
        token.transferFrom(from, to, amount);
    }

    function burnBlockedExternal(MIP20 token, address from, uint256 amount) external {
        token.burnBlocked(from, amount);
    }

    function distributeRewardExternal(MIP20 token, uint256 amount) external {
        token.distributeReward(amount);
    }

    function distributeRewardAsExternal(MIP20 token, address caller, uint256 amount) external {
        vm.prank(caller);
        token.distributeReward(amount);
    }

    function claimRewardsExternal(MIP20 token) external returns (uint256) {
        return token.claimRewards();
    }

    function claimRewardsAsExternal(MIP20 token, address caller) external returns (uint256) {
        vm.prank(caller);
        return token.claimRewards();
    }
