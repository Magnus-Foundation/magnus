...
abstract contract HandlerBase is InvariantBase {
    ...
    constructor() {
        ERR_INSUFFICIENT_BALANCE = IMIP20.InsufficientBalance.selector;
        ERR_POLICY_FORBIDS = IMIP20.PolicyForbids.selector;
    ...
    /// @dev Call this in catch blocks for protocol nonce transactions (legacy tx, magnus tx with nonceKey=0)
    ...
    /// @dev Call this in catch blocks for 2D nonce transactions (magnus tx with nonceKey > 0)
    ...
    /// @notice Record CREATE success with 2D nonce (Magnus tx with nonceKey > 0)
    /// @param sender The sender address
    ...
    /// @notice Build and sign a Magnus transaction with default settings
