...
This directory contains Solidity specifications and fuzz tests for Magnus's precompile contracts. The tests are designed to run against both:

1. **Solidity reference implementations** - Using standard Foundry
2. **Rust precompile implementations** - Using Foundry with the Magnus profile
...
### Option 2: Rust Precompiles (Magnus Profile)

Run tests against the actual Rust precompile implementations:

```bash
cd tips/ref-impls
FOUNDRY_PROFILE=magnus forge test
```
...
```bash
FOUNDRY_PROFILE=magnus forge test -vvv
```

Equivalent wrapper command:

```bash
./magnus-forge test -vvv
```

Run a specific test:

```bash
FOUNDRY_PROFILE=magnus forge test --match-test test_mint
```

Equivalent wrapper command:

```bash
./magnus-forge test --match-test test_mint
```

Foundry selects profiles via `FOUNDRY_PROFILE`, so if you run Magnus-mode commands frequently you can also set it in your shell:

```bash
export FOUNDRY_PROFILE=magnus
forge test
```
