---
id: MIP-1022
title: Virtual Addresses for MIP-20 Deposit Forwarding
description: Precompile-native virtual addresses that auto-forward MIP-20 deposits to a registered master wallet, eliminating sweep transactions.
authors: Dankrad Feist, Liam Horne, Mallesh Pai, Dan Robinson
status: Scheduled
related: MIP-20, MIP-403, MIP-1028
protocolVersion: T3
---

# MIP-1022: Virtual Addresses for MIP-20 Deposit Forwarding

## Abstract

This MIP introduces **virtual addresses**: a reserved 20-byte address format that, when detected in MIP-20 recipient-bearing operations, causes the precompile to auto-credit a registered master wallet instead of the literal target address. This eliminates sweep transactions entirely for entities such as exchanges, ramps, and payment processors that generate per-user deposit addresses. Master registration is a one-time onchain call; deposit address derivation is fully offchain.
...
- **Avoid the 250,000 gas new-account cost.** Magnus charges 250,000 gas to create state for a new address on first use. With virtual addresses, deposit addresses never create onchain state, so the first transfer to a new deposit address costs the same as any other transfer.
...
MIP-1022 intentionally places the 10-byte magic sequence in the **middle** of the address instead of at the beginning. This preserves more visually useful bytes at the front and back of the address for operators and users comparing deposit addresses in wallets, explorers, etc.
...
MIP-1022 applies only to MIP-20 precompile recipient resolution for the entrypoints listed in **Transfer Path Modification**.

MIP-1022 does **not** alter MIP-20 methods that do not carry a recipient in the MIP-20 transfer path (e.g. `approve`, `burn`, `permit`) and does not alter non-MIP-20 protocol behavior.

Non-MIP-20 token transfers (e.g. ERC-20 contracts deployed on Magnus) to virtual addresses are **not** subject to MIP-1022 forwarding. Such transfers behave as standard EVM transfers to the literal address. Tokens sent this way may be irrecoverable — see **Risks and Limitations**.

`setRewardRecipient` is **not** a MIP-20 transfer-path operation and is therefore not subject to MIP-1022 recipient resolution. Implementations MUST reject virtual addresses when setting reward recipients so that rewards remain tied to canonical accounts rather than aliases.

## Reserved Virtual Address Format

Any address whose bytes `[4:14]` equal `VIRTUAL_MAGIC` is treated as a virtual address by the MIP-20 precompile.

If a MIP-20 transfer targets such an address:
...
The literal virtual address never accumulates MIP-20 balance through standard MIP-20 transfer paths.

### Reserved Address Space

Addresses matching the virtual-address format occupy a reserved MIP-20 recipient namespace. A user who happens to control an EOA or contract whose address matches this format can still exist on Magnus and can still originate ordinary EVM transactions. However, MIP-20 transfers to such an address will follow MIP-1022 recipient resolution semantics rather than crediting the literal address.

Users who control such an address SHOULD NOT use it as a normal account on Magnus.
...
A **valid master address** MUST satisfy MIP-20 recipient safety constraints:

- MUST NOT be `address(0)`
- MUST NOT itself match the virtual-address format (`VIRTUAL_MAGIC` at bytes `[4:14]`)
- MUST NOT be a MIP-20 token address (`0x20c000....` at bytes `[0:12]`)
...
interface IAddressRegistry {
    ...
    /// @notice Resolves a transfer recipient using MIP-1022 execution semantics.
...
The following existing MIP-20 entrypoints are modified to resolve the `to` (recipient) address before crediting:
...
The `from` address on `transferFrom`, `transferFromWithMemo`, and `systemTransferFrom` is **not** affected by MIP-1022 resolution.
...
2. **Token-level sender check**: apply the standard MIP-403 / MIP-1015 sender authorization rules.
3. **Token-level recipient check**: apply the standard MIP-403 / MIP-1015 recipient authorization rules to `effectiveRecipient`.
...
If any step reverts, the enclosing MIP-20 operation MUST revert atomically with no balance changes and no events.
...
1. **Resolve recipient**: compute `effectiveRecipient = resolveRecipient(to)`. If `to` is not virtual, `effectiveRecipient = to`.
2. **Token-level mint-recipient check**: apply the standard MIP-1015 mint-recipient authorization rules to `effectiveRecipient`.
...
MIP-1022 does not introduce new authorization logic in MIP-403 itself. Instead, MIP-20 transfer and mint logic MUST resolve virtual recipient addresses before invoking the existing MIP-403 / MIP-1015 checks.

Concretely, for any MIP-20 entrypoint covered by MIP-1022:
...
This preserves view/execution symmetry with the MIP-20 authorization path defined by MIP-1015: any internal MIP-20 helper such as `isTransferAuthorized(from, to)` MUST evaluate recipient authorization against the resolved master address when `to` is virtual.

`balanceOf(virtualAddress)` remains literal and MUST continue to return 0.

Contracts or integrators that need explicit resolution behavior outside the MIP-20 transfer path MAY call `resolveRecipient` on the registry.

## Event Emission

MIP-1022 does **not** introduce new transfer-path events. The registry precompile emits `MasterRegistered`, but forwarding itself is represented using **two-hop standard `Transfer` events**: one hop showing funds arriving at the virtual address, and a second hop showing funds moving from the virtual address to the resolved master. Using standard `Transfer` events (rather than a new event type) preserves compatibility with existing indexers, block explorers, and wallets that already understand MIP-20 / ERC-20 `Transfer` events — no custom integration is required to track virtual address deposits.

For transfers where the recipient is **not** a virtual address, event emission is unchanged from standard MIP-20 behavior — a single `Transfer(sender, to, amount)`.
...
If the registered master sends tokens to one of its own virtual addresses, the transfer resolves back to the master, effectively a transfer to self. The standard MIP-20 self-transfer semantics apply (no net balance change). The two-hop `Transfer` events are still emitted: `Transfer(master, virtualAddress, amount)` followed by `Transfer(virtualAddress, master, amount)`. Indexers SHOULD NOT interpret this as net inflow when `from == masterAddress` in the first hop.

## Interaction with MIP-403

Virtual address resolution happens **before** MIP-403 / MIP-1015 authorization checks. Policy evaluation uses the resolved `masterAddress`, not the literal virtual address.
...
- Policies configured on individual virtual addresses are ignored by the MIP-20 transfer path because virtual addresses have no independent canonical MIP-20 balance.

### Rejection of Virtual Addresses in Policy Operations

MIP-403 operations that accept addresses as policy members MUST reject virtual addresses rather than accepting them silently.

Implementations SHOULD use a clear, informative error indicating that virtual addresses are aliases for MIP-20 forwarding and are not valid literal policy subjects.
...
- **Nonce / transaction origination**: A contract or EOA whose address matches the virtual-address format can still exist and can still originate ordinary EVM transactions. MIP-1022 resolution applies only to the `to` field in MIP-20 precompile calls, not to transaction senders.

## Security Considerations

### 4-Byte `masterId` and 32-Bit Registration PoW

A 4-byte `masterId` would be too small if its security relied only on raw namespace size. MIP-1022 does **not** rely on that. Instead, security comes from the combination of:
...
Virtual addresses are forwarding aliases, not canonical MIP-20 holders. Using them directly in policy configuration is misleading and dangerous because the MIP-20 transfer path evaluates policies against the resolved master address.

Accordingly, MIP-403 configuration operations SHOULD reject virtual addresses with explicit errors rather than accepting them.
...
MIP-1022 still introduces a recognizable structured address format. Wallets, block explorers, and operational tooling that truncate addresses SHOULD display enough of the address to distinguish both the `masterId` and the `userTag`; ideally they SHOULD show the full address.


### Non-MIP-20 Token Loss

MIP-1022 forwarding applies exclusively to MIP-20 precompile operations. Non-MIP-20 tokens (e.g. ERC-20 contracts deployed on Magnus) transferred to a virtual address are credited to the literal virtual address by the ERC-20 contract and are irrecoverable: no recovery mechanism is defined here.

This risk is mitigated by the strong incentives for token issuers to use MIP-20 on Magnus (gas-payment eligibility, access to the payment lane, and policy support), but it remains a limitation of this design.

### Non-MIP-20 Protocol Positions Minted to Virtual Addresses
MIP-1022 changes only the MIP-20 transfer and mint entrypoints listed in this document. It does not change other protocol logic that accepts an address parameter and records ownership against that literal address.
...
In short, virtual-address forwarding is only defined for the MIP-20 paths enumerated by MIP-1022; other protocols remain literal-address systems unless they explicitly say otherwise.

### Externally-Triggerable Revert on Unregistered Virtual Addresses

MIP-1022 introduces a recipient-dependent revert: if the `to` address matches the virtual-address format but its `masterId` is not registered, the transfer reverts with `VirtualAddressUnregistered`. This is the first MIP-20 revert condition that an untrusted recipient address can induce — prior to MIP-1022, transfers could only revert due to sender-side conditions (insufficient balance, authorization failure).
...
1. **No fund loss**: A MIP-20 transfer to a virtual address MUST either credit the registered master's balance by exactly the transfer amount, or revert. Funds MUST NOT be credited to the virtual address itself or lost.
...
6. **Non-virtual path unaffected**: Transfers to addresses that do not match the virtual-address format MUST behave identically to pre-MIP-1022 semantics, with no registry lookup.
...
9. **Atomic revert behavior**: If virtual resolution fails, the enclosing MIP-20 call MUST revert with no state changes and no events.

10. **View/execution symmetry**: MIP-20 authorization logic MUST evaluate recipient authorization against the resolved master address when `to` is virtual, matching execution-time recipient resolution semantics.

11. **Policy on master**: MIP-403 / MIP-1015 authorization for virtual-forwarded transfers and mints MUST check the resolved `masterAddress`. Policies set on individual virtual addresses MUST be ignored by the MIP-20 transfer path.

12. **Policy-operation rejection**: MIP-403 configuration operations that accept literal addresses as policy subjects or members MUST reject virtual addresses.
