...
MIP-403 currently supports two policy types: `WHITELIST` and `BLACKLIST`. This MIP adds a third type:
...
The MIP403Registry storage layout:
...
The MIP403Registry interface is extended with the following:
...
This MIP requires exactly 6 replacements of `isAuthorized` calls:

### Direct Replacements

| Location | Current | Replace With |
|----------|---------|--------------|
| MIP-20 `_mint` | `isAuthorized(to)` | `isAuthorizedMintRecipient(to)` |
| MIP-20 `burnBlocked` | `isAuthorized(from)` | `isAuthorizedSender(from)` |
...
| Location | Current | Replace With |
|----------|---------|--------------|
| MIP-20 `isTransferAuthorized` | `isAuthorized(from)` | `isAuthorizedSender(from)` |
| MIP-20 `isTransferAuthorized` | `isAuthorized(to)` | `isAuthorizedRecipient(to)` |
...
- **MIP-20**: `transfer`, `transferFrom`, `transferWithMemo`, `systemTransferFrom`
- **MIP-20 Rewards**: `distributeReward`, `setRewardRecipient`, `claimRewards`
- **Stablecoin DEX**: `decrementBalanceOrTransferFrom`, `placeLimitOrder`, `swapExactAmountIn`

## MIP-20 Integration

MIP-20 tokens MUST be updated to use the new sender/recipient authorization functions:
...
function isTransferAuthorized(address from, address to) internal view returns (bool) {
    ...
    bool fromAuthorized = MIP403_REGISTRY.isAuthorizedSender(policyId, from);
    bool toAuthorized = MIP403_REGISTRY.isAuthorizedRecipient(policyId, to);
...
function _mint(address to, uint256 amount) internal {
    if (!MIP403_REGISTRY.isAuthorizedMintRecipient(transferPolicyId, to)) {
...
function cancelStaleOrder(uint128 orderId) external {
    ...
    uint64 policyId = MIP20(token).transferPolicyId();
...
This MIP is fully backward compatible:
...
6. **MIP-20 mint**: Verify that mints check `isAuthorizedMintRecipient`, not `isAuthorizedRecipient`.

7. **MIP-20 burnBlocked**: Verify that burnBlocked checks sender authorization (and allows burning from blocked senders).
