...
# MIP-1011: Enhanced Access Key Permissions

## Abstract

This MIP extends Access Keys with three permission features:
...
This MIP intentionally adds a narrow calldata rule (first ABI `address` argument equality) instead of a generic calldata policy language.
...
Supported constrained selectors in this MIP:
...
1. `target` MUST be a MIP-20 token address.
...
/// @dev If a selector rule has `recipients`, `target` MUST be MIP-20 and `selector` MUST be transfer/approve (+memo).
...
6. If any rule has `recipients = Some([..])`, `target` MUST be a MIP-20 token address.
...
7. Selector-level recipient allowlists are optional and only valid for MIP-20 targets and the constrained selectors above.
8. Selector-level recipient allowlists are checked after selector match and before call execution.
9. This MIP does not introduce generic calldata predicates, offset math, or wildcard argument matching.
...
This MIP only specifies the additional intrinsic gas delta for call scopes in handler-side `key_authorization` charging.
...
Magnus's T3 implementation rounds this overhead upward using the same scope cardinalities:
...
1. Implementations MUST reject any selector rule with a non-empty `recipients` list whose `target` is not a MIP-20 token address.
...
14. MIP-20 target verification for selector rules with recipient allowlists.
...
9. Selector rules with recipient allowlists are valid only for MIP-20 targets and only for the fixed constrained selector set.
...
21. For constrained MIP-20 selectors (`transfer`, `approve`, `transferWithMemo`), calls succeed iff calldata argument `0` is in the configured recipient set.
...
28. Reject key authorization when selector rules with recipient allowlists are used on a non-MIP-20 target.

## References

- [AccountKeychain docs](https://docs.magnus.xyz/protocol/transactions/AccountKeychain)
- [Magnus Transactions](https://docs.magnus.xyz/guide/magnus-transaction)
- [IAccountKeychain.sol](tips/ref-impls/src/interfaces/IAccountKeychain.sol)
- [GitHub Issue #1865](https://github.com/Magnus-Foundation/magnus/issues/1865) - Periodic spending limits
- [GitHub Issue #1491](https://github.com/Magnus-Foundation/magnus/issues/1491) - Destination address scoping
