...
# MIP-1031: Embed Consensus Context into the Block Header

## Abstract

Embed consensus metadata into the `MagnusHeader`.
...
5. **Encoding backward compatibility**: The `context` field MUST be the last field in `MagnusHeader`. When `None`, it MUST be omitted entirely from the RLP stream so that the encoded representation of pre‑activation headers remains unchanged and existing block hashes are preserved.
...
- RLP round-trip for `MagnusHeader` with `context: Some(...)` and `context: None`, and that a pre-activation header's hash is unchanged after the upgrade.