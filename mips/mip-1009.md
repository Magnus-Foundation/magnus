...
```
MAGNUS_EXPIRING_NONCE_KEY = uint256.max (2^256 - 1)
```

When a Magnus transaction specifies `nonceKey = uint256.max`, the protocol treats it as an expiring nonce transaction.
...
Expiring nonce transactions pay fees in MIP-20 fee tokens like any other Magnus transaction.
...
These invariants are tested in the Foundry invariant test suite (`MagnusTransactionInvariant.t.sol`):
...
**Question**: Should the spec explicitly define the hash computation (which fields, encoding) or reference the Magnus Transaction spec?
