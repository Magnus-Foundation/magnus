...
status: Approved
related: MIP-1000, MIP-1010, EIP-8037, EIP-8011, EIP-7825, EIP-7623
protocolVersion: T3
---

# MIP-1016: Exempt Storage Creation from Gas Limits
...
MIP-1000 increased storage creation costs to 250,000 gas per operation and 1,000 gas/byte for contract code. This created two problems:
...
2. **New account throughput penalty**: MIP-20 transfer to new address costs ~300,000 gas total (~70k regular + 230k state) vs ~50,000 gas to existing. At 500M payment lane gas limit:
...
The base cost per authorization is **25,000 regular gas + 225,000 state gas = 250,000 total**, matching account creation. This reverts the MIP-1000 reduction to 12,500 gas per authorization.
...
**Exception:** Expiring nonce writes (MIP-1009) use `WARM_SSTORE_RESET` (2,900 gas) with zero state gas because they are ephemeral — entries are evicted from a fixed-size circular buffer and do not contribute to permanent state growth.
...
- Regular gas is set to at least the pre-MIP-1000 (standard EVM) cost for each operation, ensuring that exempting state gas from limits never makes an operation cheaper against protocol limits than it was before MIP-1000
...
**Note**: EIP-8037 uses `tx_gas_used` in the refund and post-refund formulas, but that variable is not defined in the same code block. MIP-1016 uses `tx_gas_used_before_refund` consistently to avoid ambiguity.
...
**Note**: Magnus has two block limits — general gas limit (~25M) for contracts and payment lane limit (500M) for simple transfers. In both lanes, only regular gas counts toward the limit; state gas is exempt.

**Divergence from EIP-8037**: EIP-8037 uses a bottleneck model where `gas_used = max(block_regular_gas, block_state_gas)`, effectively capping state gas at the block gas limit. MIP-1016 instead exempts state gas entirely from block limits, relying on fixed high prices (250,000 gas per state element) as the economic deterrent for state growth.
...
**When opcode execution starts:** Always charge `GAS_CREATE` (Magnus: 32,000 regular + 468,000 state; EIP-8037: 9,000 regular + `112 × cpsb` state)
...
### MIP-20 Transfer to New Address
...
### MIP-20 Transfer to Existing Address
- Transfer logic: ~50,000 regular gas
...
- **vs MIP-1000**: ~7,150 new account transfers/block vs ~1,700 without exemption (~4x improvement)
...
15. **Regular Gas Floor**: The regular gas component of each storage creation operation MUST be at least the pre-MIP-1000 (standard EVM) cost for that operation (SSTORE: 20,000, account creation: 25,000, CREATE base: 32,000, code deposit: 200/byte)
...
This MIP adopts the **reservoir model** from [EIP-8037](https://eips.ethereum.org/EIPS/eip-8037) for transaction-level gas accounting, with the following Magnus-specific differences:

| Aspect | EIP-8037 | MIP-1016 |
|--------|----------|----------|
| State gas pricing | Dynamic `cost_per_state_byte` scaling with block gas limit | Fixed costs (e.g., 230,000 per slot) — Magnus uses fixed high prices for state growth protection |
| Gas cost harmonization | Harmonizes all state creation to uniform cost-per-byte | Maintains Magnus-specific pricing from MIP-1000 |
