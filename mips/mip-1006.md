---
id: MIP-1006
title: Burn At for MIP-20 Tokens
description: The burnAt function for MIP-20 tokens, enabling authorized administrators to burn tokens from any address.
...
# MIP-1006: Burn At for MIP-20 Tokens

## Abstract

This specification introduces a `burnAt` function to MIP-20 tokens, allowing holders of a new `BURN_AT_ROLE` to burn tokens from any address without transfer policy restrictions. This complements the existing `burnBlocked` function which is limited to burning from addresses blocked by the transfer policy.

## Motivation

The existing MIP-20 burn mechanisms have the following limitations:
...
A new role constant is added to MIP-20:

```solidity
bytes32 public constant BURN_AT_ROLE = keccak256("BURN_AT_ROLE");
```

This role is administered by the `DEFAULT_ADMIN_ROLE` (same as other MIP-20 roles).
...
2. **Protected Addresses**: Reverts with `ProtectedAddress` if `from` is:
   - `MIP_FEE_MANAGER_ADDRESS` (0xfeEC000000000000000000000000000000000000)
...
The `IMIP20` interface is extended with:
...
3. Revert when burning from `MIP_FEE_MANAGER_ADDRESS` (ProtectedAddress)
