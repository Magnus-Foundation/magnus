...
description: Initial gas parameters for Magnus mainnet launch including base fee pricing, payment lane capacity, and transaction gas limits.
authors: Dankrad Feist @dankrad
status: Mainnet
related: MIP-1000, Payment Lane Specification, Sub block Specification
protocolVersion: T1
---

# MIP-1010: Mainnet Gas Parameters

## Abstract

This MIP specifies the initial gas parameters for Magnus mainnet, including base fee pricing, payment lane capacity, and main transaction gas limits. These parameters are calibrated to support Magnus's target of approximately 20,000 TPS for payment transactions while maintaining economically sustainable fee levels.

## Motivation

Magnus is designed as a high-throughput blockchain optimized for stablecoin payments. To achieve this, the gas parameters must be carefully calibrated to:

1. **Enable high throughput**: Support ~20,000 TPS for payment transactions
2. **Maintain low fees**: Target 0.1 cent per standard MIP-20 transfer
...
The parameters defined in this MIP represent the initial mainnet configuration and may be adjusted through future governance processes.
...
**Rationale**:
- A standard MIP-20 transfer costs approximately 50,000 gas
- At this basefee: 50,000 gas × 20 billion attodollars/gas = 10^15 attodollars = 1,000 microdollars = $0.001
- This targets approximately **0.1 cent (1,000 microdollars) per MIP-20 transfer**

**Note on units**: Attodollars (10^-18 USD) are the gas price unit. MIP-20 tokens use 6 decimals, so 1 token unit = 1 microdollar (10^-6 USD). Conversion: attodollars / 10^12 = microdollars.
...
**Rationale**:
- At 50,000 gas per MIP-20 transfer: `500,000,000 / 50,000 = 10,000 transfers per block`
- With 500ms block time: `10,000 × 2 = 20,000 TPS` for payment transactions
- This capacity supports Magnus's target throughput for payment use cases
...
**Constraints**:
- Only transactions qualifying for the payment lane (simple MIP-20 transfers, memos, etc.) may exceed the `general_gas_limit`
...
- Accommodates deployment of maximum-size contracts (24,576 bytes per EIP-170) under MIP-1000 state creation costs:
  - Base transaction cost: 21,000 gas
  - Calldata for initcode (up to 49,152 bytes per EIP-3860): ~500,000-800,000 gas
  - CREATE base cost (MIP-1000, fixed upfront contract creation cost): 500,000 gas (replaces old 32,000)
  - Initcode execution: variable (~3,000 gas minimum)
  - Contract code storage (MIP-1000): `24,576 bytes × 1,000 gas/byte = 24,576,000 gas`
...
|-----------|-------|---------|
| Base fee | `2 × 10^10` attodollars | Target 0.1 cent (1,000 microdollars) per MIP-20 transfer |
...
|-----------|----------|-------------------------------|
| MIP-20 transfer (existing recipient) | 50,000 | $0.001 (0.1 cent / 1,000 microdollars) |
| MIP-20 transfer (new recipient) | 300,000 | $0.006 (0.6 cent / 6,000 microdollars) |
...
1. **Base Fee Invariant**: The base fee is fixed at `2 × 10^10` attodollars per protocol version and can only be changed via a hardfork upgrade. At the current base fee, a MIP-20 transfer (50,000 gas) MUST cost approximately 0.1 cent (1,000 microdollars).
...
1. **Base fee targeting**: Verify that at equilibrium, MIP-20 transfers cost approximately 0.1 cent (1,000 microdollars)
2. **Payment lane capacity**: Verify that 10,000 MIP-20 transfers can be included in a single block
