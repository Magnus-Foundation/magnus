...
all consensus p2p messages. For consensus itself, Magnus employs
...
Magnus solved problems 1 and 2 by assigning validators entries anonymous addresses.
...
**Namespace:** `addValidator` uses `b"MAGNUS_VALIDATOR_CONFIG_V2_ADD_VALIDATOR"` and `rotateValidator` uses `b"MAGNUS_VALIDATOR_CONFIG_V2_ROTATE_VALIDATOR"`.
...
Because `SSTORE` cost is high under MIP-1000, migration is done one validator at a time to reduce out-of-gas risk on large sets.
...
| V2 (pre-init) | validator | `deactivateValidator` (theoretically; in Magnus these addresses are unowned) |
