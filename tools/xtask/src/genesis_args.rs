...
use magnus_consensus_engine_config::{SigningKey, SigningShare};
use magnus_contracts::{
    ...
    precompiles::{IMIP20Factory, IValidatorConfig},
};
...
pub(crate) struct GenesisArgs {
    ...
    /// Base fee
    #[arg(long, default_value_t = MAGNUS_BASE_FEE.into())]
...
impl GenesisArgs {
    ...
    pub(crate) async fn generate_genesis(self) -> eyre::Result<(Genesis, Option<ConsensusConfig>)> {
        ...
        initialize_mip20_factory(&mut evm)?;
...
fn initialize_mip20_factory(evm: &mut MagnusEvm<CacheDB<EmptyDB>>) -> eyre::Result<()> {
    let ctx = evm.ctx_mut();
