...
contract MIP20 is IMIP20, MIP20RolesAuth {
    ...
    function setNextQuoteToken(IMIP20 newQuoteToken) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (!MagnusUtilities.isMIP20(address(newQuoteToken))) {
    ...
    function burnBlocked(address from, uint256 amount) external onlyRole(BURN_BLOCKED_ROLE) {
        ...
        if (MIP403_REGISTRY.isAuthorized(transferPolicyId, from)) {
    ...
    modifier transferAuthorized(address from, address to) {
        if (
            !MIP403_REGISTRY.isAuthorized(transferPolicyId, from)
                || !MIP403_REGISTRY.isAuthorized(transferPolicyId, to)
        ) revert PolicyForbids();
        _;
    }
    ...
    function _mint(address to, uint256 amount) internal {
        if (!MIP403_REGISTRY.isAuthorized(transferPolicyId, to)) {
            revert PolicyForbids();
        }
    ...
    function distributeReward(uint256 amount) external virtual notPaused {
        ...
        if (!MIP403_REGISTRY.isAuthorized(transferPolicyId, msg.sender)) {
            revert PolicyForbids();
        }
    ...
    function setRewardRecipient(address newRewardRecipient) external virtual notPaused {
        ...
        if (newRewardRecipient != address(0)) {
            if (
                !MIP403_REGISTRY.isAuthorized(transferPolicyId, msg.sender)
                    || !MIP403_REGISTRY.isAuthorized(transferPolicyId, newRewardRecipient)
            ) revert PolicyForbids();
        }
    ...
    function claimRewards() external virtual notPaused returns (uint256 maxAmount) {
        if (
            !MIP403_REGISTRY.isAuthorized(transferPolicyId, address(this))
                || !MIP403_REGISTRY.isAuthorized(transferPolicyId, msg.sender)
        ) {
            revert PolicyForbids();
        }
