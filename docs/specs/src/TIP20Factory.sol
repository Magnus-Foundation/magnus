...
contract MIP20Factory is IMIP20Factory {
    ...
    function createToken(
        ...
        if (!MagnusUtilities.isMIP20(address(quoteToken))) {
        ...
        // NOTE: In the actual Magnus precompile implementation, the contract is deployed
        ...
        bytes memory creationCode = vm.getCode("MIP20.sol");
        vm.etch(
            tokenAddr,
            abi.encodePacked(
        ...
        require(success, "MIP20Factory: Failed to deploy MIP20");
    ...
    function isMIP20(address token) external view returns (bool) {
        return MagnusUtilities.isMIP20(token);
    }
