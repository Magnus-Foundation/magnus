...
contract MIP20FactoryTest is BaseTest {

    function testCreateUsdToken_RevertsIf_NonUsdQuoteToken() public {
        address nonUsdTokenAddr = factory.createToken(
            "Euro Token", "EUR", "EUR", IMIP20(_PATH_USD), admin, bytes32("eur")
        );
        IMIP20 nonUsdToken = IMIP20(nonUsdTokenAddr);

        try factory.createToken("USD Token", "USD", "USD", nonUsdToken, admin, bytes32("usd")) {
            revert CallShouldHaveReverted();
        } catch (bytes memory err) {
            assertEq(err, abi.encodeWithSelector(IMIP20Factory.InvalidQuoteToken.selector));
        }
    }
    ...
    function testCreateTokenCurrencyValidation() public {
        ...
        emit IMIP20Factory.TokenCreated(
            expectedAddr, "Euro Token", "EUR", "EUR", IMIP20(_PATH_USD), admin, eurSalt
        );

        address eurTokenAddr =
            factory.createToken("Euro Token", "EUR", "EUR", IMIP20(_PATH_USD), admin, eurSalt);
        IMIP20 eurToken = IMIP20(eurTokenAddr);
        ...
        IMIP20 nonUSDToken = IMIP20(tokenAddr);
    ...
    function testCreateTokenWithValidQuoteToken() public {
        ...
        address tokenAddr = factory.createToken(
            "Test Token", "TEST", "USD", IMIP20(_PATH_USD), admin, bytes32("valid")
        );

        IMIP20 token = IMIP20(tokenAddr);
    ...
    function testCreateTokenWithInvalidQuoteTokenReverts() public {
        // Try to create token with non-MIP20 address as quote token
        try factory.createToken(
            "Test Token",
            "TEST",
            "USD",
            IMIP20(address(0x1234)), // Invalid address
            admin,
            bytes32("invalid")
        ) {
            revert CallShouldHaveReverted();
    ...
    function testCreateTokenWithZeroAddressReverts() public {
        ...
        try factory.createToken(
            "Test Token", "TEST", "USD", IMIP20(address(0)), admin, bytes32("zero")
        ) {
            revert CallShouldHaveReverted();
        } catch (bytes memory err) {
            assertEq(err, abi.encodeWithSelector(IMIP20Factory.InvalidQuoteToken.selector));
    ...
    function testIsMIP20Function() public view {
        assertTrue(factory.isMIP20(_PATH_USD));
        assertTrue(factory.isMIP20(address(token1)));
        assertFalse(factory.isMIP20(address(0)));
        assertFalse(factory.isMIP20(address(0x1234)));
        assertFalse(factory.isMIP20(0x21C0000000000000000000000000000000000000));
    }
    ...
    function testDeterministicAddressGeneration() public {
        ...
        address tokenAddr =
            factory.createToken("Token 1", "TK1", "USD", IMIP20(_PATH_USD), admin, salt);
        ...
        address tokenAddr2 =
            factory.createToken("Token 2", "TK2", "USD", IMIP20(_PATH_USD), admin, salt2);
    ...
    function testGetTokenAddress() public {
        ...
        // Deploy the token
        address actual =
            factory.createToken("Predicted", "PRED", "USD", IMIP20(_PATH_USD), admin, salt);
    ...
    function testDoubleDeployment() public {
        address tokenAddr = factory.createToken(
            "Unique Token", "UNQ", "USD", IMIP20(_PATH_USD), admin, bytes32("unique_salt")
        );
        ...
        try factory.createToken(
            ...
            assertEq(
                err, abi.encodeWithSelector(IMIP20Factory.TokenAlreadyExists.selector, tokenAddr)
            );
        }
    }
    ...
    /// @notice Fuzz test: Addresses without MIP20 prefix should be invalid
    function testFuzz_isMIP20WithInvalidPrefix(uint160 randomAddr) public view {
        // Ensure address doesn't have the MIP20 prefix
        vm.assume(bytes12(bytes20(address(randomAddr))) != 0x20c000000000000000000000);

        assertFalse(factory.isMIP20(address(randomAddr)));
    }
    ...
    function testFuzz_createTokenWithInvalidQuoteToken(address invalidQuote) public {
        // Ensure it's not a valid MIP20 address
        vm.assume(!factory.isMIP20(invalidQuote));

        // Try-catch is better for precompiles than expectRevert
        try factory.createToken(
            "Token", "TK", "USD", IMIP20(invalidQuote), admin, bytes32("fuzz")
        ) returns (
            address
        ) {
            revert CallShouldHaveReverted();
            ...
            assertEq(errorSelector, IMIP20Factory.InvalidQuoteToken.selector, "Wrong error thrown");
        }
    }

    /*==================== EDGE CASES ====================*/

    /// @notice Edge case: Zero address should not be valid MIP20
    function test_EDGE_zeroAddressNotValid() public view {
        assertFalse(factory.isMIP20(address(0)));
    }

    /// @notice Edge case: Factory address itself should not be valid MIP20
    function test_EDGE_factoryAddressNotValid() public view {
        assertFalse(factory.isMIP20(_MIP20FACTORY));
    }
    ...
    function test_EDGE_cannotCreateSelfReferencingToken() public {
        ...
        // The address is not yet a valid MIP20 until it's deployed
        assertFalse(
            factory.isMIP20(nextTokenAddr), "isMIP20 should reject undeployed token address"
        );

        // Try to create a token that references itself as the quote token
        try factory.createToken("Self Ref", "SELF", "USD", IMIP20(nextTokenAddr), admin, salt) {
            revert CallShouldHaveReverted();
        } catch (bytes memory err) {
            assertEq(err, abi.encodeWithSelector(IMIP20Factory.InvalidQuoteToken.selector));
        }
    ...
    function test_DeterministicAddressWithZeroSenderAndSalt() public {
        vm.prank(address(0));
        address tokenAddr =
            factory.createToken("Zero Token", "ZERO", "USD", IMIP20(_PATH_USD), admin, bytes32(0));
