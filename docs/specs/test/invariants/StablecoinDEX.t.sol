...
contract StablecoinDEXInvariantTest is BaseTest {
    ...
    function placeOrder(
        ...
        _ensureFunds(actor, MIP20(isBid ? address(pathUSD) : token), amount);
        ...
        // Log the action
        _logPlaceOrder(actor, orderId, amount, MIP20(token).symbol(), tick, isBid);

        if (cancel) {
            _cancelAndVerifyRefund(
                orderId, actor, token, amount, tick, isBid, actorBalanceBeforePlace
            );
            _logCancelOrder(actor, orderId, isBid, MIP20(token).symbol());
        } else {
    ...
    function withdraw(uint256 actorRnd, uint128 amount, uint256 tokenRnd) external {
        ...
        _log(
            string.concat(
                _getActorIndex(actor), " withdrew ", vm.toString(amount), " ", MIP20(token).symbol()
            )
        );
    }
        ...
        try exchange.swapExactAmountIn(
            ...
            _log(
                string.concat(
                    ...
                    vm.toString(amount),
                    " ",
                    MIP20(before.tokenIn).symbol(),
                    " -> ",
                    vm.toString(amountOut),
                    " ",
                    MIP20(before.tokenOut).symbol()
                )
            );
        } catch (bytes memory reason) {
