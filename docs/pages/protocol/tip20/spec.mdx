...
interface ITIP20 {
    ...
    // =========================================================================
    //                 ISO 20022 Payment Data Functions
    // =========================================================================

    /// @notice Transfers tokens with ISO 20022 compliant payment data
    /// @param to The recipient address
    /// @param amount The amount of tokens to transfer
    /// @param endToEndId End-to-end identification (max 35 characters per ISO 20022 Max35Text)
    /// @param purposeCode Purpose code (4 bytes, e.g., "SALA", "SUPP", "TAXS")
    /// @param remittanceInfo Remittance information (max 140 characters per ISO 20022 Max140Text)
    ...
    /// @notice Transfers tokens from another account with ISO 20022 payment data
    /// @param from The sender address
    /// @param to The recipient address
    /// @param amount The amount to transfer
    /// @param endToEndId End-to-end identification (max 35 characters)
    /// @param purposeCode Purpose code (4 bytes)
    /// @param remittanceInfo Remittance information (max 140 characters)
    /// @return True if successful
    ...
    /// @notice Mints tokens with ISO 20022 payment data (requires ISSUER_ROLE)
    /// @param to The recipient address
    ...
    /// @param endToEndId ISO 20022 EndToEndIdentification
    /// @param purposeCode ISO 20022 Purpose code (e.g., SALA, SUPP, TAXS)
    ...
    /// @notice EndToEndId exceeds maximum length per ISO 20022 Max35Text
    /// @param length The actual length provided
    /// @param maxLength The maximum allowed length (35)
    error EndToEndIdTooLong(uint256 length, uint256 maxLength);

    /// @notice RemittanceInfo exceeds maximum length per ISO 20022 Max140Text
    /// @param length The actual length provided
    /// @param maxLength The maximum allowed length (140)
...
TIP-20 supports native ISO 20022 payment data fields for enhanced payment reconciliation and interoperability with traditional financial systems. The payment data functions (`transferWithPaymentData`, `transferFromWithPaymentData`, `mintWithPaymentData`) extend standard token operations with structured payment metadata:

- **EndToEndId** (Max35Text): Unique end-to-end transaction identifier, max 35 characters
- **PurposeCode** (4 bytes): ISO 20022 external purpose code indicating the payment type
- **RemittanceInfo** (Max140Text): Unstructured remittance information, max 140 characters

Payment data is emitted as event data only (not stored in contract state) for gas efficiency. Off-chain systems can index `TransferWithPaymentData` events for payment reconciliation and compliance reporting.
...
### Field Validation
The implementation validates field lengths per ISO 20022 specifications:
- `EndToEndId` exceeding 35 characters reverts with `EndToEndIdTooLong`
- `RemittanceInfo` exceeding 140 characters reverts with `RemittanceInfoTooLong`
- Empty fields are permitted for flexibility

## TIP-403 Transfer Policies
All operations that move tokens: `transfer`, `transferFrom`, `transferWithMemo`, `transferFromWithMemo`, `transferWithPaymentData`, `transferFromWithPaymentData`, `mint`, `burn`, `mintWithMemo`, `burnWithMemo`, and `mintWithPaymentData` — enforce the token's configured TIP-403 transfer policy.
