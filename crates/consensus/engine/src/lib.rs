...
use std::sync::Arc;
use magnus_chainspec::spec::{SYSTEM_TX_ADDRESSES, SYSTEM_TX_COUNT, MagnusChainSpec};
...
impl HeaderValidator<MagnusHeader> for MagnusConsensus {
    fn validate_header(&self, header: &SealedHeader<MagnusHeader>) -> Result<(), ConsensusError> {
        ...
        if header.general_gas_limit
            != (header.gas_limit() - header.shared_gas_limit) / MAGNUS_GENERAL_GAS_DIVISOR
        {
            return Err(ConsensusError::Other(
...
impl FullConsensus<MagnusPrimitives> for MagnusConsensus {
    fn validate_block_post_execution(
        ...
        FullConsensus::<MagnusPrimitives>::validate_block_post_execution(&self.inner, block, result)
    }
}
...
mod tests {
    ...
    use magnus_chainspec::spec::ANDANTINO;
    ...
    impl TestHeaderBuilder {
        ...
        fn build(self) -> MagnusHeader {
            ...
            let general_gas_limit = self.general_gas_limit.unwrap_or_else(|| {
                (self.gas_limit - self.gas_limit / MAGNUS_SHARED_GAS_DIVISOR)
                    / MAGNUS_GENERAL_GAS_DIVISOR
            });
            ...
            MagnusHeader {
                inner: Header {
                    ...
                    base_fee_per_gas: Some(magnus_chainspec::spec::MAGNUS_BASE_FEE),
    ...
    fn test_validate_header() {
        let consensus = MagnusConsensus::new(ANDANTINO.clone());
