...
use magnus_precompile_registry::extend_magnus_precompiles;
...
impl<DB: Database, I> MagnusEvm<DB, I> {
    ...
    pub fn new(ctx: MagnusContext<DB>, inspector: I) -> Self {
        ...
        extend_magnus_precompiles(&mut precompiles, &ctx.cfg);

        Self::new_inner(Evm {
            ctx,
            inspector,
            instruction: instructions::magnus_instructions(),
            precompiles,
...
mod tests {
    ...
    use crate::{MagnusBlockEnv, MagnusEvm, MagnusInvalidTransaction, MagnusTxEnv};
    ...
    fn test_access_millis_timestamp() -> eyre::Result<()> {
        let db = CacheDB::new(EmptyDB::new());
        let mut magnus_evm = MagnusEvmFactory::default().create_evm(db, Default::default());
        let ctx = magnus_evm.ctx_mut();
        ...
        StorageCtx::enter(&mut storage, || {
            MIP20Setup::create("USD", "USD", Address::ZERO).apply()
        })?;
        drop(storage);
        ...
        let res = magnus_evm.transact_raw(tx_env.into())?;
        ...
        // Test 3: InsufficientGasForIntrinsicCost - gas_limit < intrinsic_gas
        {
            ...
            assert!(
                matches!(
                    result,
                    Err(EVMError::Transaction(
                        MagnusInvalidTransaction::InsufficientGasForIntrinsicCost {
                            gas_limit: 1000,
                            intrinsic_gas
                        }
