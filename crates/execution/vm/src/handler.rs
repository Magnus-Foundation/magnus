...
use magnus_contracts::precompiles::{
    IAccountKeychain::SignatureType as PrecompileSignatureType, MIPFeeAMMError,
};
...
use magnus_primitives::transaction::{
    PrimitiveSignature, SignatureType, MagnusSignature, calc_gas_balance_spending, validate_calls,
};

use crate::{
    MagnusBatchCallEnv, MagnusEvm, MagnusInvalidTransaction,
...
fn calculate_2d_nonce_gas(
    ...
    caller: Address,
    nonce_key: U256,
) -> Result<u64, MagnusPrecompileError> {
    ...
    fn load_fee_fields(
        ...
        if (!ctx.tx.max_balance_spending()?.is_zero() || ctx.tx.is_subblock_transaction())
            && !ctx
                ...
                .map_err(|err| EVMError::Custom(err.to_string()))?
        {
            return Err(MagnusInvalidTransaction::InvalidFeeToken(self.fee_token).into());
        }
    ...
    DB: alloy_evm::Database,
{
    ...
    fn execution(
        ...
        // Check if this is an AA transaction by checking for magnus_tx_env
        let evm_ctx = evm.ctx();
        if let Some(magnus_tx_env) = evm_ctx.tx().magnus_tx_env.as_ref() {
            ...
            if spec.is_t0() {
                ...
                if gas_limit < adjusted_gas.initial_gas {
                    return Err(MagnusInvalidTransaction::InsufficientGasForIntrinsicCost {
                        gas_limit,
    ...
    fn apply_eip7702_auth_list(&self, evm: &mut Self::Evm) -> Result<u64, Self::Error> {
        ...
        if has_aa_auth_list {
            ...
            let magnus_tx_env = tx.magnus_tx_env.as_ref().unwrap();
            let mut refunded_accounts = 0;

            for authorization in &magnus_tx_env.magnus_authorization_list {
    ...
    fn validate_against_state_and_deduct_caller(
        ...
        if !nonce_key.is_zero() {
            nonce_2d_gas = StorageCtx::enter_evm(journal, block, cfg, || {
                ...
                Ok::<_, EVMError<DB::Error, MagnusInvalidTransaction>>(gas)
            })?;
        } else {
            nonce_2d_gas = 0;
            ...
            && let Some(key_auth) = &magnus_tx_env.key_authorization
        {
            ...
            if let Some(keychain_sig) = magnus_tx_env.signature.as_keychain() {
                ...
                let access_key_addr = if let Some(override_key_id) = magnus_tx_env.override_key_id {
                    ...
                    keychain_sig
                        ...
                        .map_err(|_| MagnusInvalidTransaction::AccessKeyAuthorizationFailed {
                ...
                if access_key_addr != key_auth.key_id {
                    return Err(
                            MagnusInvalidTransaction::AccessKeyAuthorizationFailed {
            ...
            if key_auth.chain_id != 0 && key_auth.chain_id != expected_chain_id {
                return Err(MagnusInvalidTransaction::KeyAuthorizationChainIdMismatch {
            ...
            StorageCtx::enter_precompile(journal, block, cfg, |mut keychain: AccountKeychain| {
                ...
                if expiry <= current_timestamp {
                    return Err(MagnusInvalidTransaction::AccessKeyAuthorizationFailed {
                        reason: format!(
                ...
                keychain
                    ...
                    .map_err(|err| match err {
                        MagnusPrecompileError::Fatal(err) => EVMError::Custom(err),
                        err => MagnusInvalidTransaction::AccessKeyAuthorizationFailed {
            ...
            StorageCtx::enter_precompile(journal, block, cfg, |mut keychain: AccountKeychain| {
                ...
                if !is_authorizing_this_key {
                    ...
                    keychain
                        ...
                        .map_err(|e| MagnusInvalidTransaction::AccessKeyAuthorizationFailed {
        ...
        if let Err(err) = result {
            ...
            Err(match err {
                MagnusPrecompileError::MIPFeeAMMError(MIPFeeAMMError::InsufficientLiquidity(_)) => {
...
pub fn calculate_aa_batch_intrinsic_gas<'a>(
    ...
    gas.initial_gas += magnus_signature_verification_gas(signature);
    ...
    for auth in authorization_list {
        gas.initial_gas += magnus_signature_verification_gas(auth.signature());
    }
...
mod tests {
    ...
    use magnus_precompile_registry::{PATH_USD_ADDRESS, MIP_FEE_MANAGER_ADDRESS};
    ...
    fn test_get_fee_token() -> eyre::Result<()> {
        ...
        ctx.journaled_state.load_account(MIP_FEE_MANAGER_ADDRESS)?;
        ctx.journaled_state
            .sstore(
    ...
    fn test_2d_nonce_gas_schedule() {
        ...
        let gas = StorageCtx::enter_evm(&mut journal, &block, &cfg, || {
            ...
            Ok::<_, EVMError<Infallible, MagnusInvalidTransaction>>(
        ...
        for (gas_limit, should_succeed) in cases {
            ...
            let ctx = Context::mainnet()
                ...
                .with_tx(MagnusTxEnv {
                    ...
                    // Set magnus_tx_env with a call to trigger AA path
            ...
            if should_succeed {
                ...
                match err.as_invalid_tx_err() {
                    Some(MagnusInvalidTransaction::InsufficientGasForIntrinsicCost {
                        gas_limit: gl,
                        intrinsic_gas,
                    }) => {
