...
#[non_exhaustive]
pub(crate) struct MagnusReceiptBuilder;
...
/// Block executor for Magnus. Wraps an inner [`EthBlockExecutor`].
pub(crate) struct MagnusBlockExecutor<'a, DB: Database, I> {
    pub(crate) inner: EthBlockExecutor<
        'a,
        MagnusEvm<&'a mut State<DB>, I>,
        &'a MagnusChainSpec,
        MagnusReceiptBuilder,
    >,
...
where
    DB: Database,
    I: Inspector<MagnusContext<&'a mut State<DB>>>,
{
    pub(crate) fn new(
...
mod tests {
    ...
    use magnus_vm::MagnusHaltReason;
