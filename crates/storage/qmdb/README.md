...
Core QMDB abstractions and traits for Magnus.

This crate provides the foundational types and traits for QMDB storage without concrete implementations. For storage backends, see [`magnus-backend`](../backend). For thread-safe access, see [`magnus-handlers`](../handlers).
...
```rust,ignore
use magnus_qmdb::{QmdbStore, ChangeSet, AccountUpdate};
