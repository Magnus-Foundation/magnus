...
Thread-safe database handles for Magnus.
...
```rust,ignore
use magnus_handlers::QmdbHandle;
...
let db = magnus_handlers::QmdbRefDb::new(handle).expect("tokio runtime");
...
1. **Layer 1 (magnus-qmdb)**: Pure store logic, state transitions, no synchronization
