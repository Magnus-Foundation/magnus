...
use magnus_crypto::{ThresholdScheme, threshold_schemes};
use magnus_domain::{
...
use magnus_reporters::{BlockContextProvider, FinalizedReporter, SeedReporter};
use magnus_simplex::{DEFAULT_MAILBOX_SIZE as MAILBOX_SIZE, DefaultPool, DefaultQuota};
use magnus_transport_sim::{SimContext, SimControl, SimLinkConfig, register_node_channels};
use thiserror::Error;
...
async fn start_all_nodes(
    ...
    bootstrap: &magnus_domain::BootstrapConfig,
    chain_id: u64,
    gas_limit: u64,
    ...
    R: Reporter<Activity = commonware_consensus::marshal::Update<Block>> + Send + 'static,
{
    ...
    let (actor, mailbox, _last_processed_height) =
        magnus_marshal::ActorInitializer::init_with_partition::<_, Block, _, _, _, Exact>(
            ctx.clone(),
...
impl<S> TestApplication<S> {
    ...
    ) -> BTreeSet<magnus_consensus::TxId> {
