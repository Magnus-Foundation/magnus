...
impl MagnusTxEnvelope {
    ...
    ///
    /// Currently uses classifier v1: transaction is a payment if the `to` address has the MIP20 prefix.
    pub fn is_payment(&self) -> bool {
        match self {
            Self::Legacy(tx) => tx
                ...
                .to
                .to()
                .is_some_and(|to| to.starts_with(&MIP20_PAYMENT_PREFIX)),
            Self::Eip2930(tx) => tx
                .tx()
                .to
                .to()
            ...
            Self::Eip1559(tx) => tx
                ...
                .to()
                .is_some_and(|to| to.starts_with(&MIP20_PAYMENT_PREFIX)),
            Self::Eip7702(tx) => tx.tx().to.starts_with(&MIP20_PAYMENT_PREFIX),
...
#[cfg(feature = "reth")]
impl reth_primitives_traits::SignedTransaction for MagnusTxEnvelope {}

#[cfg(feature = "reth")]
impl reth_primitives_traits::InMemorySize for MagnusTxType {
    fn size(&self) -> usize {
        size_of::<Self>()
    }
}
...
impl reth_primitives_traits::serde_bincode_compat::RlpBincode for MagnusTxEnvelope {}

#[cfg(feature = "reth-codec")]
mod codec {
    use crate::{MagnusSignature, MagnusTransaction};
        ...
        fn from_tx_compact(
            ...
            match tx_type {
                ...
                MagnusTxType::AA => {
                    ...
                    // For Magnus transactions, we need to decode the signature bytes as MagnusSignature
        ...
        fn to_tx_compact(&self, buf: &mut (impl BufMut + AsMut<[u8]>)) {
            match self {
                ...
                Self::AA(tx) => {
                    ...
                    // Also encode the MagnusSignature as Bytes
...
mod tests {
    use super::*;
    use crate::transaction::{Call, MagnusTransaction};
    ...
    #[test]
    fn test_payment_classification_aa_with_mip20_prefix() {
    ...
    #[test]
    fn test_payment_classification_aa_without_mip20_prefix() {
        let non_payment_addr = address!("1234567890123456789012345678901234567890");
        let call = Call {
            to: TxKind::Call(non_payment_addr),
            value: U256::ZERO,
