...
use crate::metrics::MagnusPayloadBuilderMetrics;
...
use magnus_primitives::{
    RecoveredSubBlock, SubBlockMetadata, MagnusHeader, MagnusPrimitives, MagnusTxEnvelope,
...
{
    type Attributes = MagnusPayloadBuilderAttributes;
    ...
    Provider: StateProviderFactory + ChainSpecProvider<ChainSpec = MagnusChainSpec>,
{
    ...
    fn build_payload<Txs>(
        &self,
        args: BuildArguments<MagnusPayloadBuilderAttributes, EthBuiltPayload<MagnusPrimitives>>,
        best_txs: impl FnOnce(BestTransactionsAttributes) -> Txs,
        empty: bool,
    ) -> Result<BuildOutcome<EthBuiltPayload<MagnusPrimitives>>, PayloadBuilderError>
    where
        Txs: BestTransactions<Item = Arc<ValidPoolTransaction<MagnusPooledTransaction>>>,
    {
        ...
        let general_gas_limit = non_shared_gas_limit / MAGNUS_GENERAL_GAS_DIVISOR;
...
pub fn is_more_subblocks(
    best_payload: Option<&EthBuiltPayload<MagnusPrimitives>>,
    subblocks: &[RecoveredSubBlock],
) -> bool {
...
mod tests {
    ...
    use magnus_primitives::{Block, SignedSubBlock, SubBlock, SubBlockVersion};
    ...
    fn payload_with_metadata(count: usize) -> EthBuiltPayload<MagnusPrimitives> {
