...
use magnus_primitives::RecoveredSubBlock;
...
///
/// The `MagnusPayloadBuilderAttributes` has an additional feature of interrupting payload.
///
/// It also carries DKG data to be included in the block's extra_data field.
#[derive(derive_more::Debug, Clone)]
pub struct MagnusPayloadBuilderAttributes {
...
impl MagnusPayloadBuilderAttributes {
    /// Creates new `MagnusPayloadBuilderAttributes` with `inner` attributes.
    pub fn new(
        id: PayloadId,
        parent: B256,
...
impl From<EthPayloadBuilderAttributes> for MagnusPayloadBuilderAttributes {
...
impl PayloadBuilderAttributes for MagnusPayloadBuilderAttributes {
    type RpcPayloadAttributes = MagnusPayloadAttributes;
...
/// Magnus RPC payload attributes configuration.
...
mod tests {
    ...
    impl TestExt for MagnusPayloadBuilderAttributes {
        fn random() -> Self {
            Self::new(
                PayloadId::default(),
                B256::random(),
    ...
    fn test_builder_attributes_construction() {
        ...
        // With extra_data
        let attrs = MagnusPayloadBuilderAttributes::new(
            id,
            parent,
            recipient,
            timestamp_millis,
        ...
        // Without extra_data
        let attrs2 = MagnusPayloadBuilderAttributes::new(
            id,
            parent,
            recipient,
    ...
    fn test_builder_attributes_interrupt_integration() {
        let attrs = MagnusPayloadBuilderAttributes::random();
    ...
    fn test_builder_attributes_timestamp_handling() {
        ...
        let attrs = MagnusPayloadBuilderAttributes::random().with_timestamp(3000);
        ...
        let attrs = MagnusPayloadBuilderAttributes::random().with_timestamp(3999);
        assert_eq!(attrs.timestamp(), 3);
        assert_eq!(attrs.timestamp_millis_part(), 999);
        assert_eq!(attrs.timestamp_millis(), 3999);

        // Zero timestamp
        let attrs = MagnusPayloadBuilderAttributes::random().with_timestamp(0);
        ...
        let attrs = MagnusPayloadBuilderAttributes::random().with_timestamp(large_ts + 500);
        assert_eq!(attrs.timestamp_millis_part(), 500);
    ...
    fn test_builder_attributes_subblocks() {
        ...
        let attrs = MagnusPayloadBuilderAttributes::random().with_subblocks(move || {
    ...
    fn test_from_eth_payload_builder_attributes() {
        ...
        let magnus_attrs: MagnusPayloadBuilderAttributes = eth_attrs.clone().into();

        // Inner fields preserved
        assert_eq!(magnus_attrs.payload_id(), eth_attrs.id);
        assert_eq!(magnus_attrs.parent(), eth_attrs.parent);
        ...
        assert_eq!(magnus_attrs.withdrawals().len(), 1);
        assert_eq!(
    ...
    fn test_try_new_from_rpc_attributes() {
        let rpc_attrs = MagnusPayloadAttributes {
            inner: EthPayloadAttributes {
                timestamp: 100,
                prev_randao: B256::random(),
                suggested_fee_recipient: Address::random(),
        ...
        let parent = B256::random();
        let result = MagnusPayloadBuilderAttributes::try_new(parent, rpc_attrs, 3);
        assert!(result.is_ok());
