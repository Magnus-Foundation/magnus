...
Interactive Distributed Key Generation (DKG) for Magnus threshold cryptography.
...
```bash
magnus dkg --peers /path/to/peers.json --data-dir /path/to/data
```

### Force Restart (ignore persisted state)

```bash
magnus dkg --peers /path/to/peers.json --data-dir /path/to/data --force-restart
```
