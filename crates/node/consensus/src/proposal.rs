...
use magnus_domain::{Block, StateRoot, Tx};
...
fn block_context(height: u64, prevrandao: B256) -> BlockContext {
    let header = Header {
        number: height,
        timestamp: height,
        gas_limit: magnus_config::DEFAULT_GAS_LIMIT,
...
mod tests {
    ...
    use magnus_executor::ExecutionOutcome;
    ...
    impl magnus_traits::StateDbRead for MockStateDb {
        async fn nonce(&self, _address: &Address) -> Result<u64, magnus_traits::StateDbError> {
            Ok(0)
        }

        async fn balance(&self, _address: &Address) -> Result<U256, magnus_traits::StateDbError> {
            Ok(U256::ZERO)
        }

        async fn code_hash(&self, _address: &Address) -> Result<B256, magnus_traits::StateDbError> {
            Ok(B256::ZERO)
        }

        async fn code(&self, _code_hash: &B256) -> Result<Bytes, magnus_traits::StateDbError> {
            Ok(Bytes::new())
        }
        ...
        async fn storage(
            ...
            _address: &Address,
            _slot: &U256,
        ) -> Result<U256, magnus_traits::StateDbError> {
            Ok(U256::ZERO)
        }
    }
    ...
    impl BlockExecutor<MockStateDb> for MockExecutor {
        ...
        fn execute(
            ...
            txs: &[Self::Tx],
        ) -> Result<ExecutionOutcome, magnus_executor::ExecutionError> {
            Ok(ExecutionOutcome {
        ...
        fn validate_header(&self, _header: &Header) -> Result<(), magnus_executor::ExecutionError> {
            Ok(())
        }
    }
    ...
    fn parent_block() -> Block {
        Block {
            parent: magnus_domain::BlockId(B256::ZERO),
            height: 0,
