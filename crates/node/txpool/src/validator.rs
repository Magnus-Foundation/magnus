...
use magnus_domain::Tx;
use magnus_traits::StateDbRead;
...
mod tests {
    ...
    impl magnus_traits::StateDbRead for MockState {
        async fn nonce(&self, address: &Address) -> Result<u64, magnus_traits::StateDbError> {
            Ok(*self.nonces.read().get(address).unwrap_or(&0))
        }

        async fn balance(&self, address: &Address) -> Result<U256, magnus_traits::StateDbError> {
            Ok(*self.balances.read().get(address).unwrap_or(&U256::ZERO))
        }

        async fn code_hash(&self, _: &Address) -> Result<B256, magnus_traits::StateDbError> {
            Ok(B256::ZERO)
        }

        async fn code(&self, _: &B256) -> Result<Bytes, magnus_traits::StateDbError> {
            Ok(Bytes::new())
        }

        async fn storage(&self, _: &Address, _: &U256) -> Result<U256, magnus_traits::StateDbError> {
            Ok(U256::ZERO)
        }
    }
