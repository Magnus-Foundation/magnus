...
use magnus_chainspec::spec::MAGNUS_BASE_FEE;
use magnus_precompile_registry::NONCE_PRECOMPILE_ADDRESS;

type Ordering = CoinbaseTipOrdering<MagnusPooledTransaction>;
...
impl AA2dPool {
        ...
        let tx = AA2dInternalTransaction {
            inner: PendingTransaction {
                ...
                priority: CoinbaseTipOrdering::default()
                    .priority(&transaction.transaction, MAGNUS_BASE_FEE),
    ...
    ) -> Vec<<MagnusPooledTransaction as PoolTransaction>::Pooled> {
