...
use crate::{MagnusTransactionPool, transaction::MagnusPooledTransaction};
...
use magnus_primitives::{AASigned, MagnusPrimitives};
...
where
    P: TransactionPool<Transaction = MagnusPooledTransaction> + 'static,
...
/// An endless future that maintains the [`MagnusTransactionPool`] 2d nonce pool based on the storage changes of the `NonceManager` precompile.
///
