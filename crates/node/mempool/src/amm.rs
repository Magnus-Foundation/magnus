...
use magnus_precompile_registry::{
    DEFAULT_FEE_TOKEN, MIP_FEE_MANAGER_ADDRESS,
    mip_fee_manager::{
        MipFeeManager,
...
impl AmmLiquidityCache {
    ...
    where
        Client: StateProviderFactory + BlockReader + ChainSpecProvider<ChainSpec = MagnusChainSpec>,
    {
        let this = Self {
    ...
    pub fn has_enough_liquidity(
        ...
        for validator_token in missing_in_cache {
            ...
            let slot = MipFeeManager::new().pools[pool_key].base_slot();
            let pool = state_provider
                .storage(MIP_FEE_MANAGER_ADDRESS, slot.into())?
                .unwrap_or_default();
    ...
    pub fn on_new_state(&self, execution_outcome: &ExecutionOutcome<MagnusReceipt>) {
        let Some(storage) = execution_outcome
            .account_state(&MIP_FEE_MANAGER_ADDRESS)
            .map(|acc| &acc.storage)
        else {
            return;
        };
        ...
        P: StateProviderFactory + ChainSpecProvider<ChainSpec: MagnusHardforks>,
    {
        ...
        let preference = if let Some(cached) = cached_preference {
            ...
            state
                ...
                .storage(MIP_FEE_MANAGER_ADDRESS, validator_token_slot.into())?
                .unwrap_or_default()
                .into_address()
        };
