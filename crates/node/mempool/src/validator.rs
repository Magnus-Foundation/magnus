...
use magnus_primitives::{
    ...
    transaction::{RecoveredMagnusAuthorization, MagnusTransaction},
};
use magnus_vm::{MagnusBatchCallEnv, MagnusStateAccess, calculate_aa_batch_intrinsic_gas};
    ...
    Client: ChainSpecProvider<ChainSpec = MagnusChainSpec> + StateProviderFactory,
{
    ...
    fn ensure_aa_intrinsic_gas(
        ...
        // Build the MagnusBatchCallEnv needed for gas calculation
        ...
        let init_and_floor_gas =
            calculate_aa_batch_intrinsic_gas(&aa_env, Some(tx.access_list.iter()))
                .map_err(|_| MagnusPoolTransactionError::NonZeroValue)?;
        ...
        if gas_limit < init_and_floor_gas.initial_gas {
            return Err(
                MagnusPoolTransactionError::InsufficientGasForAAIntrinsicCost {
                    gas_limit,
    ...
    fn validate_one(
        ...
        match self.validate_against_keychain(&transaction, &state_provider) {
            ...
            Ok(Err(reason)) => {
                return TransactionValidationOutcome::Invalid(
                    transaction,
                    InvalidPoolTransactionError::other(MagnusPoolTransactionError::Keychain(reason)),
                );
            }
            Err(err) => {
        ...
        // Check added in https://github.com/magnusxyz/magnus/pull/759
        ...
        if !transaction.inner().value().is_zero() {
            return TransactionValidationOutcome::Invalid(
                transaction,
                InvalidPoolTransactionError::other(MagnusPoolTransactionError::NonZeroValue),
            );
        }
                ...
                if !valid {
                    return TransactionValidationOutcome::Invalid(
                        ...
                        InvalidPoolTransactionError::other(
                            MagnusPoolTransactionError::InvalidFeeToken(fee_token),
                        ),
                    );
                }
            }
            ...
            Ok(valid) => {
                if !valid {
                    return TransactionValidationOutcome::Invalid(
                        transaction,
                        InvalidPoolTransactionError::other(
                            MagnusPoolTransactionError::BlackListedFeePayer {
                                fee_token,
                                fee_payer,
                            },
                        ),
            ...
            .has_enough_liquidity(fee_token, cost, &state_provider)
        {
            ...
            Ok(false) => {
                return TransactionValidationOutcome::Invalid(
                    transaction,
                    InvalidPoolTransactionError::other(
                        MagnusPoolTransactionError::InsufficientLiquidity(fee_token),
                    ),
                );
            }
            Err(err) => {
            ...
            .validate_one_with_state_provider(origin, transaction, &state_provider)
        {
            TransactionValidationOutcome::Valid {
                ...
                // Pre-compute MagnusTxEnv to avoid the cost during payload building.
...
mod tests {
    ...
    use magnus_precompile_registry::mip403_registry::MIP403Registry;
    ...
    fn get_transaction(with_value: Option<U256>) -> MagnusPooledTransaction {
        ...
        let mut tx = MagnusTxEnvelope::decode_2718(&mut data.as_ref()).unwrap();

        if let Some(value) = with_value {
            match &mut tx {
                MagnusTxEnvelope::Legacy(tx) => tx.tx_mut().value = value,
                MagnusTxEnvelope::Eip2930(tx) => tx.tx_mut().value = value,
                MagnusTxEnvelope::Eip1559(tx) => tx.tx_mut().value = value,
                MagnusTxEnvelope::Eip7702(tx) => tx.tx_mut().value = value,
                // set value to first call
                MagnusTxEnvelope::AA(tx) => {
        ...
        MagnusPooledTransaction::new(tx.try_into_recovered().unwrap())
    }
    ...
    > {
        ...
        let validator = MagnusTransactionValidator::new(inner, 3600, amm_cache);
    ...
    async fn test_blacklisted_fee_payer_rejected() {
        ...
        use magnus_precompile_registry::{
            ...
            mip403_registry::{IMIP403Registry, PolicyData},
        };
        use magnus_primitives::transaction::{
            MagnusTransaction,
            magnus_transaction::Call,
        ...
        // Use a valid MIP20 token address (PATH_USD with token_id=1)
        let fee_token = address!("20C0000000000000000000000000000000000001");
        let policy_id: u64 = 2;

        // Create AA transaction with valid MIP20 fee_token
        let tx_aa = MagnusTransaction {
            chain_id: 1,
            max_priority_fee_per_gas: 1_000_000_000,
            max_fee_per_gas: 2_000_000_000,
        ...
        let envelope: MagnusTxEnvelope = signed_tx.into();
        let recovered = envelope.try_into_recovered().unwrap();
        let transaction = MagnusPooledTransaction::new(recovered);
        ...
        // Add MIP20 token with transfer_policy_id pointing to blacklist policy
        ...
        // Add MIP403Registry with blacklist policy containing fee_payer
        let policy_data = PolicyData {
            policy_type: IMIP403Registry::PolicyType::BLACKLIST as u8,
            admin: Address::ZERO,
        };
        let policy_data_slot = MIP403Registry::new().policy_data[policy_id].base_slot();
        let policy_set_slot = MIP403Registry::new().policy_set[policy_id][fee_payer].slot();
        ...
        let inner = EthTransactionValidatorBuilder::new(provider.clone())
            ...
            .build(InMemoryBlobStore::default());
        let validator =
            MagnusTransactionValidator::new(inner, 3600, AmmLiquidityCache::new(provider).unwrap());
