...
use magnus_contracts::precompiles::{IAddressRegistry, IMIP20, IMIP403Registry, MIP20Error};
use magnus_precompiles::{
    ADDRESS_REGISTRY_ADDRESS, MIP403_REGISTRY_ADDRESS,
...
async fn test_tip20_transfer() -> eyre::Result<()> {
    ...
    for (_, wallet, balance) in &account_data {
        let account_provider = ProviderBuilder::new()
            .wallet(wallet.clone())
            .connect_http(http_url.clone());
        let account_token = IMIP20::new(*token.address(), account_provider);
        ...
        assert_eq!(
            result.as_decoded_interface_error::<MIP20Error>(),
            Some(MIP20Error::InsufficientBalance(
                IMIP20::InsufficientBalance {
                    available: *balance,
    ...
    for (account, wallet, _) in account_data.iter() {
        ...
        let account_provider = ProviderBuilder::new()
            .wallet(wallet.clone())
            .connect_http(http_url.clone());
        let token = IMIP20::new(*token.address(), account_provider);
    ...
    for (pending_tx, sender_balance, recipient, recipient_balance) in tx_data.into_iter() {
        ...
        let transfer_events: Vec<_> = receipt
            .logs()
            .iter()
            .filter_map(|log| IMIP20::Transfer::decode_log(&log.inner).ok())
            .collect();
        assert!(
...
async fn test_tip20_mint() -> eyre::Result<()> {
    ...
    for (tx, (account, expected_balance)) in pending_txs.drain(..).zip(account_data.iter()) {
        ...
        let mint_event = receipt
            .logs()
            .iter()
            .filter_map(|log| IMIP20::Mint::decode_log(&log.inner).ok())
            .next()
    ...
    assert_eq!(
        err.as_decoded_interface_error::<MIP20Error>(),
        Some(MIP20Error::SupplyCapExceeded(IMIP20::SupplyCapExceeded {}))
    );
...
async fn test_tip20_transfer_from() -> eyre::Result<()> {
    ...
    for (wallet, allowance) in account_data.iter() {
        ...
        let spender_provider = ProviderBuilder::new()
            .wallet(wallet.clone())
            .connect_http(http_url.clone());
        let spender_token = IMIP20::new(*token.address(), spender_provider);
...
async fn test_tip20_transfer_with_memo() -> eyre::Result<()> {
    ...
    let memo_event = receipt
        .logs()
        .iter()
        .filter_map(|log| IMIP20::TransferWithMemo::decode_log(&log.inner).ok())
        .next()
        .unwrap();
...
async fn test_tip20_blacklist() -> eyre::Result<()> {
    ...
    let registry = IMIP403Registry::new(MIP403_REGISTRY_ADDRESS, provider.clone());

    // Create a blacklist policy
    let policy_receipt = registry
        .createPolicy(admin, IMIP403Registry::PolicyType::BLACKLIST)
        .gas_price(MAGNUS_T1_BASE_FEE as u128)
        .gas(1_000_000)
        .send()
        .await?
    ...
    let policy_id = policy_receipt
        .logs()
        .iter()
        .filter_map(|log| IMIP403Registry::PolicyCreated::decode_log(&log.inner).ok())
        .next()
    ...
    for account in blacklisted_accounts {
        let provider = ProviderBuilder::new()
            ...
            .connect_http(http_url.clone());
        let token = IMIP20::new(*token.address(), provider);
...
async fn test_tip20_whitelist() -> eyre::Result<()> {
    ...
    let policy_receipt = registry
        .createPolicy(admin, IMIP403Registry::PolicyType::WHITELIST)
        .gas_price(MAGNUS_T1_BASE_FEE as u128)
        .gas(1_000_000)
        .send()
        .await?
    ...
    let whitelisted_senders: Vec<_> = whitelisted_senders
        ...
        .map(|account| {
            let provider = ProviderBuilder::new()
                .wallet(account.clone())
                .connect_http(http_url.clone());
            IMIP20::new(*token.address(), provider)
        })
        .collect();
...
async fn test_tip20_rewards() -> eyre::Result<()> {
    ...
    let alice_provider = ProviderBuilder::new()
        .wallet(alice_wallet)
        .connect_http(http_url.clone());
    let alice_token = IMIP20::new(*token.address(), alice_provider);
    ...
    let bob_provider = ProviderBuilder::new()
        .wallet(bob_wallet)
        .connect_http(http_url.clone());
    let bob_token = IMIP20::new(*token.address(), bob_provider);

    let mint_amount = U256::from(1000e18);
    let reward_amount = U256::from(300e18);

    // MIP-1000 increased state creation costs significantly (SSTORE 250k, new account 250k)
    let gas = 2_000_000;
    ...
    distribute_receipt
        .logs()
        .iter()
        .filter_map(|log| IMIP20::RewardDistributed::decode_log(&log.inner).ok())
        .next()
...
async fn test_tip20_pause_blocks_fee_collection() -> eyre::Result<()> {
    ...
    use magnus_precompiles::{MAGNUS_USD_ADDRESS, MIP_FEE_MANAGER_ADDRESS, mip20::PAUSE_ROLE};
    ...
    let token = setup_test_token(admin_provider.clone(), admin).await?;
    let user_token = IMIP20::new(*token.address(), user_provider.clone());
    ...
    let fee_amm = ITIPFeeAMM::new(MIP_FEE_MANAGER_ADDRESS, admin_provider.clone());
    fee_amm
...
async fn setup_virtual_test() -> eyre::Result<(
    ...
    Url,
    Address,
    IMIP20::IMIP20Instance<impl Provider + Clone>,
    Address,
)> {
    let setup = TestNodeBuilder::new().build_http_only().await?;
...
async fn test_tip20_virtual_transfer() -> eyre::Result<()> {
    ...
    let sender_token = IMIP20::new(*token.address(), sender_provider);
