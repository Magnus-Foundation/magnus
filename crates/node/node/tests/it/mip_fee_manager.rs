...
async fn test_fee_payer_tx() -> eyre::Result<()> {
    ...
    assert!(
        IMIP20::new(fee_payer_token, &provider)
    ...
    let balance_before = IMIP20::new(fee_payer_token, provider.clone())
    ...
    let balance_after = IMIP20::new(fee_payer_token, &provider)
        .balanceOf(fee_payer.address())
        .call()
        .await?;
...
async fn test_fee_payer_transfer_whitelist_post_t1c() -> eyre::Result<()> {
    ...
    let policy_receipt = registry
        .createPolicy(admin_addr, IMIP403Registry::PolicyType::WHITELIST)
        .gas(1_000_000)
        .send()
        .await?
        .get_receipt()
    ...
    let admin_token_contract = IMIP20::new(token_addr, &provider);
