...
async fn test_create_token() -> eyre::Result<()> {
    ...
    assert_eq!(event.address, MIP20_FACTORY_ADDRESS);
    ...
    // Verify the token address has MIP20 prefix
    assert!(event.token.is_tip20(), "Token should have MIP20 prefix");

    let token = IMIP20::new(event.token, provider);
...
async fn test_is_tip20_checks_code_deployment() -> eyre::Result<()> {
    ...
    // Create an address with valid MIP20 prefix but no code deployed
    ...
    // Verify this address has valid MIP20 prefix
    assert!(
        non_existent_tip20_addr.is_tip20(),
        "Address should have valid MIP20 prefix"
    );
