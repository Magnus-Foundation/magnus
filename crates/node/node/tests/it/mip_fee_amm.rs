...
async fn test_transact_different_fee_tokens() -> eyre::Result<()> {
    ...
    let validator_token = IMIP20Instance::new(PATH_USD_ADDRESS, provider.clone());
    ...
    // Transfer using predeployed MIP20
    let transfer_token = IMIP20::new(DEFAULT_FEE_TOKEN, provider.clone());
    ...
    // in debug mode. Related, MIP20 transfers do not update the `to` address balance if it is
