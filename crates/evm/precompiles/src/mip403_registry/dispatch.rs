//! ABI dispatch for the [`MIP403Registry`] precompile.
...
use magnus_contracts::precompiles::IMIP403Registry::{self, IMIP403RegistryCalls};

const T2_ADDED: &[[u8; 4]] = &[
    IMIP403Registry::isAuthorizedSenderCall::SELECTOR,
    IMIP403Registry::isAuthorizedRecipientCall::SELECTOR,
    IMIP403Registry::isAuthorizedMintRecipientCall::SELECTOR,
    IMIP403Registry::compoundPolicyDataCall::SELECTOR,
    IMIP403Registry::createCompoundPolicyCall::SELECTOR,
];
    ...
    fn call(&mut self, calldata: &[u8], msg_sender: Address) -> PrecompileResult {
        ...
        dispatch_call(
            ...
            |call| match call {
                ...
                IMIP403RegistryCalls::isAuthorized(call) => view(call, |c| {
                    self.is_authorized_as(c.policyId, c.user, AuthRole::Transfer)
                }),
                // MIP-1015: T2+ only (gated via T2_ADDED_SELECTORS)
                IMIP403RegistryCalls::isAuthorizedSender(call) => view(call, |c| {
                    self.is_authorized_as(c.policyId, c.user, AuthRole::Sender)
                }),
                IMIP403RegistryCalls::isAuthorizedRecipient(call) => view(call, |c| {
                    self.is_authorized_as(c.policyId, c.user, AuthRole::Recipient)
                }),
                IMIP403RegistryCalls::isAuthorizedMintRecipient(call) => view(call, |c| {
                    self.is_authorized_as(c.policyId, c.user, AuthRole::MintRecipient)
                }),
                IMIP403RegistryCalls::compoundPolicyData(call) => {
                ...
                // MIP-1015: T2+ only (gated via T2_ADDED_SELECTORS)
                IMIP403RegistryCalls::createCompoundPolicy(call) => {
...
mod tests {
    ...
    fn test_selector_coverage() -> eyre::Result<()> {
        // Use T2 to test all selectors including MIP-1015 compound policy functions
