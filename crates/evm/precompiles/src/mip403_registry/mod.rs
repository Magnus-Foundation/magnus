//! [MIP-403] transfer policy registry precompile.
//!
//! Manages whitelist, blacklist, and compound transfer policies that MIP-20
//! tokens reference to gate sender/recipient authorization.
//!
//! [MIP-403]: <https://docs.magnus.xyz/protocol/mip403>

pub mod dispatch;

use crate::StorageCtx;
pub use magnus_contracts::precompiles::{
    IMIP403Registry::{self, PolicyType},
    MIP403RegistryError, MIP403RegistryEvent,
};
...
/// Registry for [MIP-403] transfer policies. MIP20 tokens reference an ID from this registry
...
#[contract(addr = MIP403_REGISTRY_ADDRESS)]
...
/// Policy record containing base data and optional data for compound policies ([MIP-1015])
///
...
/// Data for compound policies ([MIP-1015])
///
/// [MIP-1015]: <https://docs.magnus.xyz/protocol/mips/mip-1015>
...
impl PolicyData {
    ...
    fn policy_type(&self) -> Result<PolicyType> {
        ...
        match self.policy_type.try_into() {
            ...
            _ => Err(if is_t2 {
                MIP403RegistryError::invalid_policy_type().into()
            } else {
...
impl MIP403Registry {
    /// Initializes the MIP-403 registry precompile.
    ...
    pub fn policy_exists(&self, call: IMIP403Registry::policyExistsCall) -> Result<bool> {
    ...
    pub fn policy_data(
        &self,
        call: IMIP403Registry::policyDataCall,
    ) -> Result<IMIP403Registry::policyDataReturn> {
        if self.storage.spec().is_t2() {
            ...
            if self.builtin_authorization(call.policyId).is_some() {
                return Ok(IMIP403Registry::policyDataReturn {
                    policyType: (call.policyId as u8)
                        .try_into()
                        .map_err(|_| MIP403RegistryError::invalid_policy_type())?,
                    admin: Address::ZERO,
                });
            }
        } else {
            // Check if policy exists before reading the data (spec: pre-T2)
            if !self.policy_exists(IMIP403Registry::policyExistsCall {
                policyId: call.policyId,
            })? {
                return Err(MIP403RegistryError::policy_not_found().into());
            }
        }
        ...
        Ok(IMIP403Registry::policyDataReturn {
            policyType: data.policy_type()?,
            admin: data.admin,
        })
    }

    /// Returns the sub-policy IDs of a compound policy ([MIP-1015]).
    ///
    /// [MIP-1015]: <https://docs.magnus.xyz/protocol/mips/mip-1015>
    ///
    /// # Errors
    ...
    pub fn compound_policy_data(
        &self,
        call: IMIP403Registry::compoundPolicyDataCall,
    ) -> Result<IMIP403Registry::compoundPolicyDataReturn> {
        let data = self.get_policy_data(call.policyId)?;

        // Only compound policies have compound data
        if !data.is_compound() {
            // Check if the policy exists for error clarity
            let err = if self.policy_exists(IMIP403Registry::policyExistsCall {
                policyId: call.policyId,
            })? {
                MIP403RegistryError::incompatible_policy_type()
            } else {
                MIP403RegistryError::policy_not_found()
            };
            return Err(err.into());
        }

        let compound = self.policy_records[call.policyId].compound.read()?;
        Ok(IMIP403Registry::compoundPolicyDataReturn {
    ...
    pub fn create_policy(
        &mut self,
        msg_sender: Address,
        call: IMIP403Registry::createPolicyCall,
    ) -> Result<u64> {
        ...
        self.emit_event(MIP403RegistryEvent::PolicyCreated(
            IMIP403Registry::PolicyCreated {
        ...
        self.emit_event(MIP403RegistryEvent::PolicyAdminUpdated(
            IMIP403Registry::PolicyAdminUpdated {
                policyId: new_policy_id,
                updater: msg_sender,
                admin: call.admin,
            },
    ...
    pub fn create_policy_with_accounts(
        &mut self,
        msg_sender: Address,
        call: IMIP403Registry::createPolicyWithAccountsCall,
    ) -> Result<u64> {
        let admin = call.admin;
        let policy_type = call.policyType.ensure_is_simple()?;

        // MIP-1022: reject virtual addresses in initial account set (spec T3+)
        if self.storage.spec().is_t3() {
            for account in call.accounts.iter() {
                if account.is_virtual() {
                    return Err(MIP403RegistryError::virtual_address_not_allowed().into());
                }
            }
        }
        ...
        for account in call.accounts.iter() {
            ...
            match call.policyType {
                PolicyType::WHITELIST => {
                    self.emit_event(MIP403RegistryEvent::WhitelistUpdated(
                        IMIP403Registry::WhitelistUpdated {
                            policyId: new_policy_id,
                            updater: msg_sender,
                            account: *account,
                            allowed: true,
                ...
                PolicyType::BLACKLIST => {
                    self.emit_event(MIP403RegistryEvent::BlacklistUpdated(
                        IMIP403Registry::BlacklistUpdated {
                            policyId: new_policy_id,
                            updater: msg_sender,
                            account: *account,
                            restricted: true,
                ...
                IMIP403Registry::PolicyType::COMPOUND | IMIP403Registry::PolicyType::__Invalid => {
                    // T2+: unreachable since `ensure_is_simple` already rejected
                    return Err(MIP403RegistryError::incompatible_policy_type().into());
                }
            }
        }
    ...
    pub fn set_policy_admin(
        &mut self,
        msg_sender: Address,
        call: IMIP403Registry::setPolicyAdminCall,
    ) -> Result<()> {
        let data = self.get_policy_data(call.policyId)?;

        // Check authorization
        if data.admin != msg_sender {
            return Err(MIP403RegistryError::unauthorized().into());
        }
    ...
    pub fn modify_policy_whitelist(
        &mut self,
        msg_sender: Address,
        call: IMIP403Registry::modifyPolicyWhitelistCall,
    ) -> Result<()> {
        // MIP-1022: virtual addresses are forwarding aliases, not valid policy members (spec: T3+)
    ...
    pub fn modify_policy_blacklist(
        &mut self,
        msg_sender: Address,
        call: IMIP403Registry::modifyPolicyBlacklistCall,
    ) -> Result<()> {
        // MIP-1022: virtual addresses are forwarding aliases, not valid policy members (spec: T3+)
        if self.storage.spec().is_t3() && call.account.is_virtual() {
            return Err(MIP403RegistryError::virtual_address_not_allowed().into());
    ...
    /// Creates a new compound policy that references three simple sub-policies ([MIP-1015]).
    ...
    pub fn create_compound_policy(
        &mut self,
        msg_sender: Address,
        call: IMIP403Registry::createCompoundPolicyCall,
    ) -> Result<u64> {
        ...
        // Emit event
        self.emit_event(MIP403RegistryEvent::CompoundPolicyCreated(
            IMIP403Registry::CompoundPolicyCreated {
                policyId: new_policy_id,
                creator: msg_sender,
    ...
    /// Core role-based authorization check ([MIP-1015]). Resolves built-in policies (0 = reject,
    ...
    fn is_simple(&self, policy_id: u64, user: Address, data: &PolicyData) -> Result<bool> {
        ...
        match data.policy_type()? {
            ...
            PolicyType::COMPOUND => Err(MIP403RegistryError::incompatible_policy_type().into()),
    ...
    fn validate_simple_policy(&self, policy_id: u64) -> Result<()> {
        ...
        let data = self.get_policy_data(policy_id)?;
        if !data.is_simple() {
            return Err(MIP403RegistryError::policy_not_simple().into());
        }
...
pub fn is_policy_lookup_error(e: &MagnusPrecompileError) -> bool {
    if StorageCtx.spec().is_t2() {
        // T2+: typed MIP403 errors
        *e == MIP403RegistryError::invalid_policy_type().into()
            || *e == MIP403RegistryError::policy_not_found().into()
    } else {
    ...
    fn ensure_is_simple(&self) -> Result<u8> {
        match self {
            ...
            Self::COMPOUND | Self::__Invalid => {
                if StorageCtx.spec().is_t2() {
                    Err(MIP403RegistryError::incompatible_policy_type().into())
                } else {
...
mod tests {
    ...
    use magnus_contracts::precompiles::MIP403_REGISTRY_ADDRESS;
    ...
    fn test_create_policy() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            // Check policy data
            let data = registry.policy_data(IMIP403Registry::policyDataCall { policyId: 2 })?;
            assert_eq!(data.policyType, IMIP403Registry::PolicyType::WHITELIST);
    ...
    fn test_policy_data_reverts_for_non_existent_policy() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = registry.policy_data(IMIP403Registry::policyDataCall { policyId: 100 });
            assert!(result.is_err());

            // Verify the error is PolicyNotFound
            assert!(matches!(
                result.unwrap_err(),
                MagnusPrecompileError::MIP403RegistryError(MIP403RegistryError::PolicyNotFound(_))
            ));
    ...
    fn test_policy_data_builtin_policies_boundary() -> eyre::Result<()> {
        for (hardfork, expect_allow_all_type) in [
            ...
            (MagnusHardfork::T1C, IMIP403Registry::PolicyType::WHITELIST),
            // T2: virtual builtins return correct types
            (MagnusHardfork::T2, IMIP403Registry::PolicyType::BLACKLIST),
        ] {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                let reject = registry.policy_data(IMIP403Registry::policyDataCall {
                    policyId: REJECT_ALL_POLICY_ID,
                })?;
                assert_eq!(reject.policyType, IMIP403Registry::PolicyType::WHITELIST);
                assert_eq!(reject.admin, Address::ZERO);

                // allow-all → WHITELIST pre-T2 (wrong), BLACKLIST from T2 (correct)
                let allow = registry.policy_data(IMIP403Registry::policyDataCall {
    ...
    fn test_policy_exists() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(registry.policy_exists(IMIP403Registry::policyExistsCall { policyId: 0 })?);
            assert!(registry.policy_exists(IMIP403Registry::policyExistsCall { policyId: 1 })?);

            // Test 100 random policy IDs > 1 should not exist initially
            let mut rng = rand_08::thread_rng();
            for _ in 0..100 {
                let random_policy_id = rng.gen_range(2..u64::MAX);
                assert!(!registry.policy_exists(IMIP403Registry::policyExistsCall {
    ...
    // =========================================================================
    //                      MIP-1015: Compound Policy Tests
    // =========================================================================
    ...
    fn test_create_compound_policy() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let data = registry.policy_data(IMIP403Registry::policyDataCall {
                policyId: compound_id,
            })?;
            assert_eq!(data.policyType, IMIP403Registry::PolicyType::COMPOUND);
            assert_eq!(data.admin, Address::ZERO); // Compound policies have no admin

            // Verify compound policy data
            let compound_data =
                registry.compound_policy_data(IMIP403Registry::compoundPolicyDataCall {
                    policyId: compound_id,
                })?;
    ...
    fn test_policy_data_rejects_compound_policy_on_pre_t1() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = registry.policy_data(IMIP403Registry::policyDataCall {
                policyId: compound_id,
            });
            assert!(result.is_err());
    ...
    fn test_create_policy_rejects_non_simple_policy_types() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            for policy_type in [
                IMIP403Registry::PolicyType::COMPOUND,
                IMIP403Registry::PolicyType::__Invalid,
            ] {
                let result = registry.create_policy(
                    admin,
                    IMIP403Registry::createPolicyCall {
                ...
                assert!(matches!(
                    result.unwrap_err(),
                    MagnusPrecompileError::MIP403RegistryError(
                        MIP403RegistryError::IncompatiblePolicyType(_)
                    )
                ));
            }
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            for policy_type in [
                ...
                let result = registry.create_policy_with_accounts(
                    admin,
                    IMIP403Registry::createPolicyWithAccountsCall {
                        admin,
                        policyType: policy_type,
    ...
    fn test_pre_t1_create_policy_with_accounts_invalid_type_behavior() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let policy_id = registry.create_policy_with_accounts(
                ...
                IMIP403Registry::createPolicyWithAccountsCall {
                    admin,
                    policyType: IMIP403Registry::PolicyType::__Invalid,
                    accounts: vec![],
                },
            )?;
            let data = registry.get_policy_data(policy_id)?;
    ...
    fn test_t2_compound_policy_rejects_legacy_invalid_255_policy() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(matches!(
                result.unwrap_err(),
                MagnusPrecompileError::MIP403RegistryError(MIP403RegistryError::PolicyNotSimple(_))
            ));
    ...
    fn test_pre_t1_create_policy_event_emits_invalid() -> eyre::Result<()> {
        ...
        let events = storage.events.get(&MIP403_REGISTRY_ADDRESS).unwrap();
        let policy_created_log = Log::new_unchecked(
            MIP403_REGISTRY_ADDRESS,
            events[0].topics().to_vec(),
            events[0].data.clone(),
        );
        let decoded = IMIP403Registry::PolicyCreated::decode_log(&policy_created_log)?;

        // should emit 255, not 2
        assert_eq!(decoded.policyType, IMIP403Registry::PolicyType::__Invalid);
    ...
    fn test_t2_create_policy_emits_correct_type() -> eyre::Result<()> {
        ...
        let whitelist_decoded = IMIP403Registry::PolicyCreated::decode_log(&whitelist_log)?;
        assert_eq!(
        ...
        let blacklist_decoded = IMIP403Registry::PolicyCreated::decode_log(&blacklist_log)?;
        assert_eq!(
    ...
    fn test_compound_policy_data_error_cases() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = registry
                .compound_policy_data(IMIP403Registry::compoundPolicyDataCall { policyId: 999 });
            assert!(matches!(
                result.unwrap_err(),
                MagnusPrecompileError::MIP403RegistryError(MIP403RegistryError::PolicyNotFound(_))
            ));
            ...
            let result = registry.compound_policy_data(IMIP403Registry::compoundPolicyDataCall {
    ...
    fn test_initialize_sets_storage_state() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let registry2 = MIP403Registry::new();
    ...
    fn test_policy_exists_boundary_at_counter() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(
                !registry.policy_exists(IMIP403Registry::policyExistsCall { policyId: counter })?
            );
    ...
    fn test_nonexistent_policy_behavior() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(
                ...
                MIP403RegistryError::policy_not_found().into()
            );
            assert_eq!(
                registry
    ...
    // ────────────────── MIP-1022 Virtual Address Rejection ──────────────────
    ...
    fn test_modify_whitelist_rejects_virtual_address() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(matches!(
                result.unwrap_err(),
                MagnusPrecompileError::MIP403RegistryError(
                    MIP403RegistryError::VirtualAddressNotAllowed(_)
                )
            ));
