//! Opt-in staking [rewards system] for MIP-20 tokens.
//!
//! Token holders opt in by setting a reward recipient via [`MIP20Token::set_reward_recipient`].
...
//!
//! [Reward system]: <https://docs.magnus.xyz/protocol/mip20-rewards/overview>

use crate::{
    error::{Result, MagnusPrecompileError},
    storage::Handler,
    mip20::{Recipient, MIP20Token},
};
use alloy::primitives::{Address, U256, uint};
use magnus_contracts::precompiles::{IMIP20, MIP20Error, MIP20Event};
...
impl MIP20Token {
    ...
    /// - `PolicyForbids` — MIP-403 policy rejects the transfer
    ...
    pub fn distribute_reward(
        &mut self,
        msg_sender: Address,
        call: IMIP20::distributeRewardCall,
    ) -> Result<()> {
        self.check_not_paused()?;
        let token_address = self.address;

        if call.amount == U256::ZERO {
            return Err(MIP20Error::invalid_amount().into());
        }
        ...
        if opted_in_supply.is_zero() {
            return Err(MIP20Error::no_opted_in_supply().into());
        }
        ...
        self.emit_event(MIP20Event::RewardDistributed(IMIP20::RewardDistributed {
            funder: msg_sender,
            amount: call.amount,
        }))?;
    ...
    /// # Errors
    /// - `Paused` — token transfers are currently paused
    /// - `PolicyForbids` — MIP-403 policy rejects the sender→recipient transfer authorization
    /// - `InvalidRecipient` — MIP-1022 virtual addresses are rejected
    pub fn set_reward_recipient(
        &mut self,
        msg_sender: Address,
        call: IMIP20::setRewardRecipientCall,
    ) -> Result<()> {
        self.check_not_paused()?;

        // MIP-1022: reject virtual addresses as reward recipients
        ...
        self.emit_event(MIP20Event::RewardRecipientSet(IMIP20::RewardRecipientSet {
            holder: msg_sender,
    ...
    /// # Errors
    /// - `Paused` — token transfers are currently paused
    /// - `PolicyForbids` — MIP-403 policy rejects the contract→caller transfer authorization
...
impl From<UserRewardInfo> for IMIP20::UserRewardInfo {
...
mod tests {
    ...
    use magnus_contracts::precompiles::{IMIP403Registry, MIP20Error};
    ...
    fn test_set_reward_recipient() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token
                .set_reward_recipient(alice, IMIP20::setRewardRecipientCall { recipient: alice })?;
    ...
    fn test_distribute_reward() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result =
                token.distribute_reward(admin, IMIP20::distributeRewardCall { amount: U256::ZERO });
    ...
    fn test_get_pending_rewards_with_delegation() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token.set_reward_recipient(alice, IMIP20::setRewardRecipientCall { recipient: bob })?;
        ...
        for hardfork in [MagnusHardfork::T2, MagnusHardfork::T3] {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                if hardfork.is_t3() {
                    assert!(matches!(
                        result.unwrap_err(),
                        MagnusPrecompileError::MIP20(MIP20Error::InvalidRecipient(_))
                    ));
                } else {
