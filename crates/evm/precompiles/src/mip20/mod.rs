//! [MIP-20] token standard — Magnus's native fungible token implementation.
//!
//! Provides ERC-20-like balances, allowances, and transfers with Magnus extensions:
//! role-based access control, pausability, supply caps, transfer policies ([MIP-403]), opt-in
//! staking rewards, EIP-2612 permits (T2+), quote-token graphs, and virtual addresses ([MIP-1022]).
//!
...
pub use magnus_contracts::precompiles::{
    IRolesAuth, IMIP20, RolesAuthError, RolesAuthEvent, MIP20Error, MIP20Event, USD_CURRENCY,
};
...
use magnus_contracts::precompiles::DECIMALS as MIP20_DECIMALS;
...
/// features: role-based access control, pausability, supply caps, transfer policies ([MIP-403]),
/// virtual addresses ([MIP-1022]), and opt-in staking rewards.
///
/// [MIP-403]: <https://docs.magnus.xyz/protocol/mip403>
/// [MIP-1022]: <https://docs.magnus.xyz/protocol/mip1022>
///
...
impl MIP20Token {
    ...
    /// Returns the token decimals (always 6 for MIP-20).
    ...
    /// Returns the MIP-403 transfer policy ID governing this token's transfers.
    ...
    /// Updates the [`MIP403Registry`] transfer policy governing this token's transfers.
    ///
    /// # Errors
    /// - `Unauthorized` — caller does not hold `DEFAULT_ADMIN_ROLE`
    /// - `InvalidTransferPolicyId` — policy does not exist in the [`MIP403Registry`]
    ...
    /// Validates that the candidate is a deployed MIP-20 token (via [`MIP20Factory`]) and, for
    ...
    pub fn set_next_quote_token(
        ...
        if !MIP20Factory::new().is_tip20(call.newQuoteToken)? {
            return Err(MIP20Error::invalid_quote_token().into());
        }
    ...
    /// - Enforces mint-recipient compliance via [`MIP403Registry`] and validates against supply cap
    ...
    /// - `InvalidRecipient` — (+T3) recipient is zero or a MIP-20 prefix address
    /// - `PolicyForbids` — MIP-403 policy rejects the mint recipient
    ...
    /// Burns tokens from addresses blocked by [`MIP403Registry`] policy.
    ///
    /// # Errors
    /// - `ContractPaused` — (+T3) token is paused
    ...
    pub fn burn_blocked(
        ...
        if MIP403Registry::new().is_authorized_as(policy_id, call.from, AuthRole::sender())? {
    ...
    pub fn nonces(&self, call: IMIP20::noncesCall) -> Result<U256> {
    ...
    pub fn permit(&mut self, call: IMIP20::permitCall) -> Result<()> {
        // 1. Check deadline
        if self.storage.timestamp() > call.deadline {
            return Err(MIP20Error::permit_expired().into());
        }
        ...
        // Only v=27/28 is accepted; v=0/1 is intentionally NOT normalized (see MIP-1004 spec).
        let recovered = self
            .storage
            .recover_signer(digest, call.v, call.r, call.s)?
            .ok_or(MIP20Error::invalid_signature())?;
        if recovered != call.owner {
            return Err(MIP20Error::invalid_signature().into());
        }
    ...
    /// [`MIP403Registry`] and deducts from the caller's [`AccountKeychain`] spending limit.
    ///
    /// # Errors
    /// - `Paused` — token transfers are currently paused
    /// - `InvalidRecipient` — recipient address is zero
    /// - `PolicyForbids` — MIP-403 policy rejects sender or recipient
    ...
    /// Enforces compliance via the [`MIP403Registry`].
    ///
    /// # Errors
    /// - `Paused` — token transfers are currently paused
    /// - `InvalidRecipient` — recipient address is zero
    ...
    /// compliance via the [`MIP403Registry`] and [`AccountKeychain`].
    ///
    /// # Errors
    /// - `Paused` — token transfers are currently paused
    /// - `InvalidRecipient` — recipient address is zero
...
// Utility functions
impl MIP20Token {
    /// Creates a `MIP20Token` handle from a raw address.
    ///
    /// # Errors
    /// - `InvalidToken` — address does not carry the `0x20C0` MIP-20 prefix
    ...
    /// Creates a MIP20Token without validating the prefix.
    ///
    /// # Safety
    /// Caller must ensure `is_tip20_prefix(address)` returns true.
    #[inline]
    pub fn from_address_unchecked(address: Address) -> Self {
        debug_assert!(address.is_tip20(), "address must have MIP20 prefix");
        Self::__new(address)
    }

    /// Initializes the MIP-20 token precompile with metadata, quote token, supply cap, and
    /// default admin role. Called once by [`MIP20Factory`] during token creation.
    pub fn initialize(
        &mut self,
        msg_sender: Address,
        name: &str,
    ...
    fn validate_mint(&self, to: &Recipient) -> Result<()> {
        ...
        // Check if the resolved target address is authorized to receive minted tokens
        if !MIP403Registry::new().is_authorized_as(
    ...
    /// Check whether a transfer is authorized by the token's [`MIP403Registry`] policy.
    /// [MIP-1015]: For T2+, uses directional sender/recipient checks.
    ///
    ...
    /// Ensures the transfer is authorized by the token's [`MIP403Registry`] policy.
    ///
    /// # Errors
...
/// Resolved transfer recipient for [MIP-1022] virtual address support.
///
...
impl Recipient {
    ...
    /// - an address with the MIP-20 prefix (preventing transfers to token contracts)
    ...
    pub(crate) fn build_transfer_event(&self, from: Address, amount: U256) -> MIP20Event {
    ...
    pub(crate) fn build_virtual_transfer_event(&self, amount: U256) -> Option<MIP20Event> {
...
mod recipient_tests {
    ...
    fn test_build_events() {
        ...
        assert!(matches!(direct.build_transfer_event(from, amount),
            MIP20Event::Transfer(IMIP20::Transfer { to, .. }) if to == target));
        assert!(matches!(virt.build_transfer_event(from, amount),
            MIP20Event::Transfer(IMIP20::Transfer { to, .. }) if to == vaddr));
        ...
        assert!(matches!(hop,
            MIP20Event::Transfer(IMIP20::Transfer { from, to, .. })
...
pub(crate) mod tests {
    ...
    use crate::{
        ...
        error::MagnusPrecompileError,
        storage::{StorageCtx, hashmap::HashMapStorageProvider},
        test_util::{MIP20Setup, VIRTUAL_MASTER, register_virtual_master, setup_storage},
    };
    ...
    fn test_new_invalid_quote_token() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            MIP20Setup::create("USD Token", "USDT", admin)
                ...
                .expect_tip20_err(MIP20Error::invalid_quote_token());
    ...
    // ═══════════════════════════════════════════════════════════
    //  MIP-1022 Virtual Address Tests
    // ═══════════════════════════════════════════════════════════
    ...
    fn test_unregistered_virtual_reverts_on_t3() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(token.mint(admin, IMIP20::mintCall { to, amount }).is_err());
            assert!(token.mint_with_memo(admin, IMIP20::mintWithMemoCall { to, amount, memo }).is_err());
            assert!(token.transfer(sender, IMIP20::transferCall { to, amount }).is_err());
            assert!(token.transfer_with_memo(sender, IMIP20::transferWithMemoCall { to, amount, memo }).is_err());
            assert!(token.transfer_from(spender, IMIP20::transferFromCall { from: sender, to, amount }).is_err());
            assert!(token.transfer_from_with_memo(spender, IMIP20::transferFromWithMemoCall { from: sender, to, amount, memo }).is_err());
    ...
    // ═══════════════════════════════════════════════════════════
    //  EIP-2612 Permit Tests (MIP-1004)
    // ═══════════════════════════════════════════════════════════
    ...
    mod permit_tests {
        ...
        fn test_permit_happy_path() -> eyre::Result<()> {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                let nonce = token.nonces(IMIP20::noncesCall { owner })?;
        ...
        fn test_permit_expired() -> eyre::Result<()> {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                assert!(matches!(
                    result,
                    Err(MagnusPrecompileError::MIP20(MIP20Error::PermitExpired(_)))
                ));
        ...
        fn test_permit_invalid_signature() -> eyre::Result<()> {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                let result = token.permit(IMIP20::permitCall {
                    owner,
                    spender,
                    value,
                    deadline,
                ...
                assert!(matches!(
                    result,
                    Err(MagnusPrecompileError::MIP20(MIP20Error::InvalidSignature(_)))
                ));
        ...
        fn test_permit_nonce_tracking() -> eyre::Result<()> {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                assert_eq!(token.nonces(IMIP20::noncesCall { owner })?, U256::ZERO);
                ...
                for i in 0u64..3 {
                    ...
                    assert_eq!(
                        token.nonces(IMIP20::noncesCall { owner })?,
                        U256::from(i + 1)
                    );
                }
        ...
        fn test_permit_works_when_paused() -> eyre::Result<()> {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                assert_eq!(
                    token.allowance(IMIP20::allowanceCall { owner, spender })?,
                    value
                );
        ...
        fn test_permit_domain_separator_changes_with_chain_id() -> eyre::Result<()> {
            ...
            let ds_a = StorageCtx::enter(&mut storage_a, || {
                MIP20Setup::create("Test", "TST", admin)
                    .apply()?
                    .domain_separator()
            })?;
    ...
    fn test_mint_rejects_when_paused_on_t3() -> eyre::Result<()> {
        ...
        for hardfork in [MagnusHardfork::T2, MagnusHardfork::T3] {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                let mint_result = token.mint(admin, IMIP20::mintCall { to, amount });
                let mint_memo_result =
                    token.mint_with_memo(admin, IMIP20::mintWithMemoCall { to, amount, memo });

                if hardfork.is_t3() {
                    let expected = MagnusPrecompileError::MIP20(MIP20Error::contract_paused());
    ...
    fn test_burn_rejects_when_paused_on_t3() -> eyre::Result<()> {
        ...
        for hardfork in [MagnusHardfork::T2, MagnusHardfork::T3] {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                let burn_result = token.burn(admin, IMIP20::burnCall { amount });
