pub use IMIP403Registry::{
    IMIP403RegistryErrors as MIP403RegistryError, IMIP403RegistryEvents as MIP403RegistryEvent,
};
...
impl IMIP403Registry::PolicyType {
...
impl MIP403RegistryError {
    /// Creates an error for unauthorized calls
    pub const fn unauthorized() -> Self {
        Self::Unauthorized(IMIP403Registry::Unauthorized {})
    }

    /// Creates an error for incompatible policy types
    pub const fn invalid_policy_type() -> Self {
        Self::InvalidPolicyType(IMIP403Registry::InvalidPolicyType {})
    }

    /// Creates an error for incompatible policy types
    pub const fn incompatible_policy_type() -> Self {
        Self::IncompatiblePolicyType(IMIP403Registry::IncompatiblePolicyType {})
    }

    /// Creates an error for non-existent policy
    pub const fn policy_not_found() -> Self {
        Self::PolicyNotFound(IMIP403Registry::PolicyNotFound {})
    }

    pub const fn policy_not_simple() -> Self {
        Self::PolicyNotSimple(IMIP403Registry::PolicyNotSimple {})
    }

    /// Virtual addresses are MIP-1022 forwarding aliases and cannot be used as policy members.
    pub const fn virtual_address_not_allowed() -> Self {
        Self::VirtualAddressNotAllowed(IMIP403Registry::VirtualAddressNotAllowed {})
    }
}
