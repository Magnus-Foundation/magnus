...
/// Decimal precision for all MIP-20 tokens.
...
impl IMIP20::IMIP20Calls {
    /// Returns `true` if `input` matches one of the recognized [MIP-20 payment] selectors:
    ...
    ///
    /// # NOTES
    /// - Only validates calldata; the caller must check the MIP-20 address prefix on `to`.
    ...
    pub fn is_payment(input: &[u8]) -> bool {
        ...
        is_call::<IMIP20::transferCall>(input)
            || is_call::<IMIP20::transferWithMemoCall>(input)
            || is_call::<IMIP20::transferFromCall>(input)
            || is_call::<IMIP20::transferFromWithMemoCall>(input)
            || is_call::<IMIP20::approveCall>(input)
            || is_call::<IMIP20::mintCall>(input)
            || is_call::<IMIP20::mintWithMemoCall>(input)
            || is_call::<IMIP20::burnCall>(input)
            || is_call::<IMIP20::burnWithMemoCall>(input)
    }
}
...
impl MIP20Error {
    ...
    pub const fn permit_expired() -> Self {
        Self::PermitExpired(IMIP20::PermitExpired {})
    }

    /// Error when permit signature is invalid
    pub const fn invalid_signature() -> Self {
        Self::InvalidSignature(IMIP20::InvalidSignature {})
    }
}
...
mod test {
    ...
    fn payment_calldatas() -> [Vec<u8>; 9] {
        ...
        [
            IMIP20::transferCall { to, amount }.abi_encode(),
            IMIP20::transferWithMemoCall { to, amount, memo }.abi_encode(),
            IMIP20::transferFromCall { from, to, amount }.abi_encode(),
            IMIP20::transferFromWithMemoCall { from, to, amount, memo }.abi_encode(),
            IMIP20::approveCall { spender: to, amount }.abi_encode(),
            IMIP20::mintCall { to, amount }.abi_encode(),
            IMIP20::mintWithMemoCall { to, amount, memo }.abi_encode(),
            IMIP20::burnCall { amount }.abi_encode(),
            IMIP20::burnWithMemoCall { amount, memo }.abi_encode(),
        ]
    }

    #[rustfmt::skip]
    /// Returns ABI-encoded calldata for MIP-20 selectors NOT recognized as payments.
    fn non_payment_calldatas() -> [Vec<u8>; 3] {
        let mut data = IMIP20::transferCall { to: Address::random(), amount: U256::random() }.abi_encode();
        data[..4].copy_from_slice(&[0xde, 0xad, 0xbe, 0xef]);

        [
            // non-payment MIP20 calls with known selectors
            IMIP20::claimRewardsCall {}.abi_encode(),
            IMIP20::permitCall {
            ...
            // non-payment MIP20 calls with unknown selectors
            data,
        ]
    }
    ...
    fn test_is_payment() {
        for calldata in payment_calldatas() {
            assert!(IMIP20::IMIP20Calls::is_payment(&calldata))
        }
