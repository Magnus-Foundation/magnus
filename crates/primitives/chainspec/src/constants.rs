...
pub mod gas {
    ...
    pub const MAGNUS_T0_BASE_FEE: u64 = 10_000_000_000;
    ...
    pub const MAGNUS_T1_EXISTING_NONCE_KEY_GAS: u64 = COLD_SLOAD + WARM_SSTORE_RESET;
    /// T2 adds 2 warm SLOADs for the extended nonce key lookup.
    pub const MAGNUS_T2_EXISTING_NONCE_KEY_GAS: u64 =
        MAGNUS_T1_EXISTING_NONCE_KEY_GAS + 2 * WARM_SLOAD;

    /// Gas cost for using a new 2D nonce key (cold SLOAD + SSTORE set for 0 -> non-zero).
    pub const MAGNUS_T1_NEW_NONCE_KEY_GAS: u64 = COLD_SLOAD + SSTORE_SET;
    /// T2 adds 2 warm SLOADs for the extended nonce key lookup.
    pub const MAGNUS_T2_NEW_NONCE_KEY_GAS: u64 = MAGNUS_T1_NEW_NONCE_KEY_GAS + 2 * WARM_SLOAD;
}
