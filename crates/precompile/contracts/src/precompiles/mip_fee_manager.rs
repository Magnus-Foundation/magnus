...
pub use IMIPFeeAMM::{IMIPFeeAMMErrors as MIPFeeAMMError, IMIPFeeAMMEvents as MIPFeeAMMEvent};

crate::sol! {
    /// FeeManager interface for managing gas fee collection and distribution.
    ///
    /// IMPORTANT: FeeManager inherits from MIPFeeAMM and shares the same storage layout.
    /// This means:
    /// - FeeManager has all the functionality of MIPFeeAMM (pool management, swaps, liquidity operations)
    /// - Both contracts use the same storage slots for AMM data (pools, reserves, liquidity balances)
    /// - FeeManager extends MIPFeeAMM with additional storage slots (4-15) for fee-specific data
    /// - When deployed, FeeManager IS a MIPFeeAMM with additional fee management capabilities
    ///
    /// Storage layout:
    /// - Slots 0-3: MIPFeeAMM storage (pools, pool exists, liquidity data)
...
sol! {
    /// MIPFeeAMM interface defining the base AMM functionality for stablecoin pools.
    /// This interface provides core liquidity pool management and swap operations.
    ///
    /// NOTE: The FeeManager contract inherits from MIPFeeAMM and shares the same storage layout.
    /// When FeeManager is deployed, it effectively "is" a MIPFeeAMM with additional fee management
    /// capabilities layered on top. Both contracts operate on the same storage slots.
    #[derive(Debug, PartialEq, Eq)]
    #[allow(clippy::too_many_arguments)]
    interface IMIPFeeAMM {
        // Structs
        struct Pool {
