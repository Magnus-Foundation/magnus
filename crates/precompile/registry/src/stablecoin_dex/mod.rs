...
use crate::{
    ...
    mip20::{IMIP20, MIP20Token, is_mip20_prefix, validate_usd_currency},
...
impl StablecoinDEX {
    ...
    pub fn create_pair(&mut self, base: Address) -> Result<B256> {
        ...
        if !MIP20Factory::new().is_mip20(base)? {
    ...
    /// Cancel a stale order where the maker is forbidden by MIP-403 policy
    ...
    pub fn cancel_stale_order(&mut self, order_id: u128) -> Result<()> {
        ...
        let token_contract = MIP20Token::from_address(token)?;
        let policy_id = token_contract.transfer_policy_id()?;

        let registry = MIP403Registry::new();
        let is_authorized = registry.is_authorized(IMIP403Registry::isAuthorizedCall {
            policyId: policy_id,
            user: order.maker(),
        })?;
    ...
    fn find_trade_path(&self, token_in: Address, token_out: Address) -> Result<Vec<(B256, bool)>> {
        ...
        if !is_mip20_prefix(token_in) || !is_mip20_prefix(token_out) {
    ...
    fn validate_and_build_route(&self, path: &[Address]) -> Result<Vec<(B256, bool)>> {
        ...
        for i in 0..path.len() - 1 {
            ...
            let (base, quote) = {
                let token_in_mip20 = MIP20Token::from_address(token_in)?;
                if token_in_mip20.quote_token()? == token_out {
                    (token_in, token_out)
                } else {
                    let token_out_mip20 = MIP20Token::from_address(token_out)?;
                    if token_out_mip20.quote_token()? == token_in {
                        (token_out, token_in)
                    } else {
...
mod tests {
    ...
    fn test_place_bid_order() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let quote_mip20 = MIP20Token::from_address(quote_token)?;
            let remaining_balance =
                quote_mip20.balance_of(IMIP20::balanceOfCall { account: alice })?;
            assert_eq!(remaining_balance, U256::ZERO);

            // Verify exchange received the tokens
            let exchange_balance = quote_mip20.balance_of(IMIP20::balanceOfCall {
    ...
    fn test_place_ask_order() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            // Verify balance was reduced by the escrow amount
            let base_mip20 = MIP20Token::from_address(base_token)?;
            let remaining_balance =
                base_mip20.balance_of(IMIP20::balanceOfCall { account: alice })?;
            assert_eq!(remaining_balance, U256::ZERO); // All tokens should be escrowed

            // Verify exchange received the base tokens
            let exchange_balance = base_mip20.balance_of(IMIP20::balanceOfCall {
                account: exchange.address,
            })?;
    ...
    fn test_withdraw() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(
                quote_mip20.balance_of(IMIP20::balanceOfCall {
                    account: exchange.address
                })?,
                0
            );
    ...
    fn test_swap_exact_amount_out() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let base_mip20 = MIP20Token::from_address(base_token)?;
            let bob_base_balance = base_mip20.balance_of(IMIP20::balanceOfCall { account: bob })?;
    ...
    fn test_swap_exact_amount_in() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let bob_quote_balance =
                quote_mip20.balance_of(IMIP20::balanceOfCall { account: bob })?;
    ...
    #[test]
    fn test_create_pair_rejects_non_mip20_base() -> eyre::Result<()> {
        let mut storage = HashMapStorageProvider::new(1);
        StorageCtx::enter(&mut storage, || {
            let admin = Address::random();
            ...
            let result = exchange.create_pair(non_mip20_address);
            assert!(matches!(
                result,
    ...
    #[test]
    fn test_find_trade_path_rejects_non_mip20() -> eyre::Result<()> {
        let mut storage = HashMapStorageProvider::new(1);
        StorageCtx::enter(&mut storage, || {
            let mut exchange = StablecoinDEX::new();
            exchange.initialize()?;
            ...
            let non_mip20_address = Address::random();
            let result = exchange.find_trade_path(non_mip20_address, quote_token);
            assert!(
                matches!(
                    result,
                    Err(MagnusPrecompileError::StablecoinDEX(
