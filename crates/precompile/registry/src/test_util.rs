...
impl MIP20Setup {
    ...
    /// Initialize the MIP20 factory if needed.
    ...
    /// Apply the configuration, returning just the MIP20Token.
    pub fn apply(self) -> Result<MIP20Token> {
        ...
        for (to, amount) in self.mints {
            let admin = self.admin.unwrap_or_else(|| {
                get_mip20_admin(token.address()).expect("unable to get token admin")
            });
    ...
    /// Apply the configuration, and expect it to fail with the given MIP20 error.
    pub fn expect_mip20_err(self, expected: MIP20Error) {
        let result = self.apply();
...
#[cfg(any(test, feature = "test-utils"))]
fn get_mip20_admin(token: Address) -> Option<Address> {
