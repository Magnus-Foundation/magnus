...
impl MipFeeManager {
    ...
    ///
    /// This ensures the [`MipFeeManager`] isn't empty and prevents state clear.
    ...
    pub fn set_validator_token(
        ...
        if !MIP20Factory::new().is_mip20(call.token)? {
...
mod tests {
    ...
    fn test_collect_fee_post_tx() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let token = MIP20Setup::create("Test", "TST", admin)
                .with_issuer(admin)
                .with_mint(MIP_FEE_MANAGER_ADDRESS, U256::from(100000000000000_u64))
                .apply()?;
    ...
    fn test_collect_fee_pre_tx_different_tokens() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let validator_token = MIP20Setup::create("ValidatorToken", "VTK", admin)
                .with_issuer(admin)
                .with_mint(MIP_FEE_MANAGER_ADDRESS, U256::from(10000))
                .apply()?;
    ...
    fn test_collect_fee_pre_tx_insufficient_liquidity() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let validator_token = MIP20Setup::create("ValidatorToken", "VTK", admin)
                .with_issuer(admin)
                .with_mint(MIP_FEE_MANAGER_ADDRESS, U256::from(100))
                .apply()?;
    ...
    fn test_distribute_fees() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let token = MIP20Setup::create("TestToken", "TEST", admin)
                .with_issuer(admin)
                .with_mint(MIP_FEE_MANAGER_ADDRESS, U256::from(1000))
                .apply()?;
