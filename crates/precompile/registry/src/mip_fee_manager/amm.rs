use crate::{
    ...
    storage::Handler,
    mip_fee_manager::{IMIPFeeAMM, MIPFeeAMMError, MIPFeeAMMEvent, MipFeeManager},
...
use magnus_precompile_macros::Storable;
...
impl From<Pool> for IMIPFeeAMM::Pool {
...
impl MipFeeManager {
    ...
    pub fn get_pool(&self, call: IMIPFeeAMM::getPoolCall) -> Result<Pool> {
    ...
    pub fn check_sufficient_liquidity(
        ...
        if amount_out_needed > U256::from(pool.reserve_validator_token) {
            return Err(MIPFeeAMMError::insufficient_liquidity().into());
        }
    ...
    pub fn rebalance_swap(
        ...
        if amount_out.is_zero() {
            return Err(MIPFeeAMMError::invalid_amount().into());
        }
        ...
        let amount_in: u128 = amount_in
            .try_into()
            .map_err(|_| MIPFeeAMMError::invalid_amount())?;
        ...
        pool.reserve_validator_token = pool
            ...
            .ok_or(MIPFeeAMMError::insufficient_reserves())?;

        pool.reserve_user_token = pool
            .reserve_user_token
            .checked_sub(amount_out)
            .ok_or(MIPFeeAMMError::invalid_amount())?;

        self.pools[pool_id].write(pool)?;

        let amount_in = U256::from(amount_in);
        let amount_out = U256::from(amount_out);
        MIP20Token::from_address(validator_token)?.system_transfer_from(
            msg_sender,
            self.address,
            amount_in,
        )?;

        MIP20Token::from_address(user_token)?.transfer(
            self.address,
            IMIP20::transferCall {
                to,
                amount: amount_out,
        ...
        self.emit_event(MIPFeeAMMEvent::RebalanceSwap(IMIPFeeAMM::RebalanceSwap {
    ...
    pub fn mint(
        ...
        if user_token == validator_token {
            return Err(MIPFeeAMMError::identical_addresses().into());
        }
        ...
        let liquidity = if pool.reserve_user_token == 0 && pool.reserve_validator_token == 0 {
            ...
            half_amount
                ...
                .ok_or(MIPFeeAMMError::insufficient_liquidity())?
        } else {
            ...
            let product = N
                ...
                .and_then(|product| product.checked_div(SCALE))
                .ok_or(MIPFeeAMMError::invalid_swap_calculation())?;
            ...
            if denom.is_zero() {
                return Err(MIPFeeAMMError::division_by_zero().into());
            }
        ...
        let _ = MIP20Token::from_address(validator_token)?.system_transfer_from(
            msg_sender,
            self.address,
            amount_validator_token,
        )?;
        ...
        // Emit Mint event
        self.emit_event(MIPFeeAMMEvent::Mint(IMIPFeeAMM::Mint {
            sender: msg_sender,
            to,
            userToken: user_token,
            validatorToken: validator_token,
    ...
    pub fn burn(
        ...
        let _ = MIP20Token::from_address(user_token)?.transfer(
            self.address,
            IMIP20::transferCall {
                to,
        ...
        let _ = MIP20Token::from_address(validator_token)?.transfer(
            self.address,
            IMIP20::transferCall {
                to,
        ...
        // Emit Burn event
        self.emit_event(MIPFeeAMMEvent::Burn(IMIPFeeAMM::Burn {
            sender: msg_sender,
            userToken: user_token,
            validatorToken: validator_token,
...
mod tests {
    ...
    use crate::{
        ...
        mip_fee_manager::MIPFeeAMMError,
    };
    ...
    fn test_mint_identical_addresses() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(matches!(
                result,
                Err(MagnusPrecompileError::MIPFeeAMMError(
                    MIPFeeAMMError::IdenticalAddresses(_)
                ))
            ));
            Ok(())
        })
    ...
    fn test_rebalance_swap_insufficient_funds() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(matches!(
                result,
                Err(MagnusPrecompileError::MIPFeeAMMError(
                    MIPFeeAMMError::InvalidAmount(_)
                ))
            ));
            Ok(())
        })
    ...
    fn test_mint_insufficient_amount() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(matches!(
                result,
                Err(MagnusPrecompileError::MIPFeeAMMError(
                    MIPFeeAMMError::InsufficientLiquidity(_)
                ))
            ));
            Ok(())
        })
    ...
    fn test_add_liquidity() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let token1 = MIP20Setup::create("Token1", "TK1", admin)
                .with_issuer(admin)
                .with_mint(admin, mint_amount)
                .apply()?
                .address();
            let token2 = MIP20Setup::create("Token2", "TK2", admin)
                .with_issuer(admin)
                .with_mint(admin, mint_amount)
                .apply()?
                .address();
