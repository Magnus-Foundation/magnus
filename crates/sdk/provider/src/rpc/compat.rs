...
use magnus_vm::{MagnusBatchCallEnv, MagnusTxEnv};
...
impl TryIntoTxEnv<MagnusTxEnv, MagnusBlockEnv> for MagnusTransactionRequest {
    type Err = EthApiError;

    fn try_into_tx_env<Spec>(
        self,
        evm_env: &EvmEnv<Spec, MagnusBlockEnv>,
        ...
        Ok(MagnusTxEnv {
                ...
                || key_id.is_some()
            {
                ...
                let mock_signature =
                    create_mock_magnus_signature(&key_type, key_data.as_ref(), key_id, caller_addr);
...
fn create_mock_magnus_signature(
    ...
    if key_id.is_some() {
        ...
        MagnusSignature::Keychain(KeychainSignature::new(caller_addr, inner_sig))
    } else {
