...
///
/// `N` is a nonce filler.
pub(crate) type MagnusFillers<N> = JoinFill<N, JoinFill<GasFiller, ChainIdFiller>>;
...
impl TransactionBuilder<MagnusNetwork> for MagnusTransactionRequest {
    ...
    fn complete_type(&self, ty: MagnusTxType) -> Result<(), Vec<&'static str>> {
        match ty {
            ...
            | MagnusTxType::Eip7702 => self
                .inner
                .complete_type(ty.try_into().expect("magnus tx types checked")),
        }
    }
    ...
    fn output_tx_type_checked(&self) -> Option<MagnusTxType> {
        match self.output_tx_type() {
            ...
            | MagnusTxType::Eip2930
            | MagnusTxType::Eip1559
            | MagnusTxType::Eip7702 => self.inner.output_tx_type_checked()?.try_into().ok(),
        }
    }
...
impl NetworkWallet<MagnusNetwork> for EthereumWallet {
    ...
    async fn sign_transaction_from(
        &self,
        sender: Address,
        mut tx: MagnusTypedTransaction,
    ) -> alloy_signer::Result<MagnusTxEnvelope> {
