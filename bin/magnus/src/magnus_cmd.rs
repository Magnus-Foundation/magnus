...
///
/// These commands are defined here so they appear in `magnus --help`, but
...
/// Magnus-specific subcommands that extend the reth CLI.
#[derive(Debug, Subcommand)]
pub(crate) enum MagnusSubcommand {
    ...
    ///
    /// Loads MIP20 storage slots from a binary file generated by `magnus-xtask generate-state-bloat`
    /// and applies them to the genesis state.
    InitFromBinaryDump(Box<init_state::InitFromBinaryDump<MagnusChainSpecParser>>),

    /// Install an extension (e.g., `magnus add wallet`).
    #[command(
        override_usage = "magnus add <EXT> [VERSION]",
        after_help = "Examples:\n  magnus add wallet\n  magnus add wallet 0.2.0"
    )]
    Add(ExtArgs),

    /// Update magnus and/or extensions.
    #[command(
        override_usage = "magnus update [EXT]",
    ...
    /// Remove an extension.
    #[command(
        override_usage = "magnus remove <EXT>",
        after_help = "Example: magnus remove wallet"
    )]
    Remove(ExtArgs),

    /// List installed extensions.
    #[command(override_usage = "magnus list")]
    List(ExtArgs),
}

impl ExtendedCommand for MagnusSubcommand {
        ...
        match self {
            ...
            Self::InitFromBinaryDump(cmd) => {
                ...
                runner.run_blocking_until_ctrl_c(
                    cmd.execute::<magnus_node::node::MagnusNode>(runtime),
                )?;
                Ok(())
            }
...
impl ValidatorTransactionArgs {
    ...
    async fn provider(&self) -> eyre::Result<impl Provider<MagnusNetwork>> {
...
pub(crate) struct ValidatorInfo {
    ...
    chain: Option<Arc<MagnusChainSpec>>,
...
mod tests {
    ...
    type MagnusCli = Cli<
        MagnusChainSpecParser,
        crate::MagnusArgs,
        crate::MagnusRpcModuleValidator,
        MagnusSubcommand,
    >;

    #[test]
    fn parse_p2p_proxy_defaults() {
        let cli = MagnusCli::try_parse_from([
            "magnus",
            "p2p-proxy",
            "--rpc-url",
        ...
        assert!(matches!(
            cli.command,
            reth_ethereum::cli::Commands::Ext(MagnusSubcommand::P2pProxy(_))
        ));
    }
    ...
    fn parse_p2p_proxy_missing_rpc_url_fails() {
        let result = MagnusCli::try_parse_from(["magnus", "p2p-proxy"]);
    ...
    fn magnus_rpc_module_validator_rejects_unknown_modules() {
        let err = crate::MagnusRpcModuleValidator::parse_selection("not-a-real-module").unwrap_err();
