...
use magnus_consensus_engine::feed::FeedStateHandle;
...
pub async fn launch_execution_node<P: AsRef<Path>>(
    ...
    let magnus_node = MagnusNode::default().with_validator_key(config.validator_key);
...
fn setup_magnus_evm() -> MagnusEvm<CacheDB<EmptyDB>> {
